    ...
    <div className="mx-auto max-w-2xl">
      ...
      <section className={`${panel} rise overflow-hidden`} style={{ ["--i" as string]: 1 }}>
        ...
        <ul className="divide-y divide-stone-100 dark:divide-stone-800">
          {open.map((t) => <Row key={t.id} t={t} onToggle={() => toggle.mutate({ id: t.id, done: true })} onRemove={() => remove.mutate(t.id)} />)}
        </ul>
      </section>
      ...
      {done.length > 0 && (
        <section className="rise mt-5" style={{ ["--i" as string]: 2 }}>
          ...
          <ul className={`${panel} divide-y divide-stone-100 overflow-hidden dark:divide-stone-800`}>
            {done.map((t) => <Row key={t.id} t={t} onToggle={() => toggle.mutate({ id: t.id, done: false })} onRemove={() => remove.mutate(t.id)} />)}
          </ul>
        </section>
      )}
      <p className="mt-6 text-center text-xs text-stone-400">Also from Claude Code: “add ‘book the scanner’ to my list”.</p>
    </div>
  );
}

function Row({ t, onToggle, onRemove }: { t: Todo; onToggle: () => void; onRemove: () => void }) {
  return (
    <li className="group flex items-center gap-3 px-4 py-3">
      ...
      <span className={`min-w-0 flex-1 text-base transition-colors ${t.done ? "text-stone-400 line-through" : "text-stone-800 dark:text-stone-100"}`}>{t.text}</span>
