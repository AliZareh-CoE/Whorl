...
type Calibration = { count: number; on_time: number; median_late: number | null; p80_late: number | null; median_late_first: number | null; buckets: Record<string, number>; worst: { id: number; title: string; late: number } | null; shift: number | null; min_sample: number }; // #519
type PlanData = { project: string; project_name: string; project_color: string; questions: Question[]; phases: Phase[]; conflicts: Conflict[]; critical_chain: Chain; drift: Drift; review: ReviewState; calibration: Calibration };
...
export default function Plan() {
  ...
  return (
    <div>
      ...
      {(data.calibration?.count ?? 0) > 0 && mode !== "roadmap" && !reviewing && (
        <p className="rise mb-4 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-xs text-stone-500 dark:text-stone-400" data-testid="plan-calibration" title="Every completed milestone that held a date is a sample: the days between the date it held and the day it was done. The median is your habit; Atlas adds it to every open date as its likely landing.">
          <Crosshair className="h-3.5 w-3.5 shrink-0 text-stone-400" aria-hidden="true" />
          <span className="shrink-0 font-medium text-stone-700 dark:text-stone-200">Calibration</span>
          <span className={`shrink-0 rounded-md px-1.5 py-0.5 ${(data.calibration.median_late ?? 0) > 0 ? "bg-amber-500/10 text-amber-800 dark:text-amber-200" : "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"}`}>{(data.calibration.median_late ?? 0) > 0 ? `milestones land a median ${data.calibration.median_late} d late` : (data.calibration.median_late ?? 0) < 0 ? `milestones land a median ${-(data.calibration.median_late ?? 0)} d early` : "milestones land on the day"}</span>
          <span className="shrink-0">· {data.calibration.on_time} of {data.calibration.count} on time</span>
          {data.calibration.median_late_first != null && data.calibration.median_late_first !== data.calibration.median_late && <span className="shrink-0">· {data.calibration.median_late_first > 0 ? `${data.calibration.median_late_first} d late` : data.calibration.median_late_first < 0 ? `${-data.calibration.median_late_first} d early` : "on the day"} against the first dates</span>}
          {data.calibration.worst && <span className="min-w-0 truncate">· <span className="font-medium text-stone-700 dark:text-stone-200">{data.calibration.worst.title}</span> landed latest (+{data.calibration.worst.late} d)</span>}
          <span className="shrink-0 text-stone-400">{data.calibration.shift == null ? `· likely dates appear after ${data.calibration.min_sample} landings` : data.calibration.shift > 0 ? `· open dates carry a likely landing ${data.calibration.shift} d later` : data.calibration.shift < 0 ? `· open dates carry a likely landing ${-data.calibration.shift} d earlier` : "· your dates hold — no likely shift"}</span>
        </p>
      )}
