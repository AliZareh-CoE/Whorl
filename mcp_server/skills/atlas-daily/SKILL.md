...
4. When the user agrees, `add_todo` for each (short imperative text). Do not add more.
   ...
   "not today" on an existing item is `snooze_todo` (`until=""` brings it back); a wrong one
   is `trash_todo` (thirty days in the Trash, `restore=True` brings it back). Something
