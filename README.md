...
- Start from the object, not the prompt: a paper's menu (Library or Reference page), a manuscript's menu and the note editor offer **Use a prompt with this…** — the gallery opens showing only the prompts that take that kind, the object already filled in, one click from Copy. Claude asks the same with `list_prompts(kind="reference")`.
