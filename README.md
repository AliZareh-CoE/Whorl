...
Claude Code can do all of it with you — 99 tools over the same API the UI uses.
...
Tools — projects & plans: `get_dashboard`, `get_diagnostics`, `get_achievements`, `list_projects`, `get_project_overview`, `get_plan`,
...
`list_manuscript_files`, `read_manuscript_file`, `write_manuscript_file`, `attach_manuscript_figure` (a local PNG/PDF into the source tree, with the `\includegraphics` snippet back), `set_main_file`,
