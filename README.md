...
- Edit a text file in the pane with a real editor: **Edit** opens a CodeMirror editor (line numbers, undo, search, Markdown and LaTeX highlighting, `⌘S`), a one-line note says what changed, and the save lands as a new version with the previous text kept — a **Saved as v3 · +4 −1 lines** strip shows the diff right there (a `.csv` opens with **Edit as text** and the strip shows the changed cells); unsaved changes are guarded when you switch files or leave the page. `PUT /api/v1/documents/{id}/content/` answers `version` and `filed`, and `write_project_file` answers the same so Claude can read back what it changed with `read_project_file(version=filed, diff=True)`.
...
- Deleting a file is safe: it goes into a **Trash** under the explorer (no confirm — the toast undoes it), where it keeps its bytes, its tags and its version history for thirty days; **Restore** puts it back into the folder it left (the project root when that folder is gone, a numbered name when a new file took its path), the × or **Empty the trash** deletes for good, and the nightly sweep does the same after thirty days. Manuscript sources never sit here (the Studio owns them). `DELETE /api/v1/documents/{id}/` trashes (`?forever=true`, or a second DELETE, deletes for good), `POST …/documents/{id}/restore/` restores, `GET /api/v1/documents/?project=&trash=true` lists the Trash, `POST /api/v1/projects/{slug}/documents/empty-trash/` empties it; `list_project_files` carries `trash` and `organize_files(paths, "delete" | "restore" | "purge")` is the same for Claude.
- Only emptying the Trash (or the sweep) removes a file's bytes and its versions' bytes from disk; `manage.py prune_media` (dry run; `--apply`) sweeps document files no row references — a trashed file is still referenced.
