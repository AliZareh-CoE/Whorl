...
Claude Code can do all of it with you — 141 tools over the same API the UI uses.
...
`set_reading_status`, `browse_library`, `check_retractions`, `check_preprints`, `upgrade_preprint`, `get_reading_progress`, `set_reading_position`, `run_bib_check`, `get_review_matrix`, `set_review_mark`, `add_review_theme`, `suggest_review_themes`, `get_synthesis_scaffold`.
