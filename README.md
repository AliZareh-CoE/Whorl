...
- Files carry tags and a description in the explorer: coloured chips and a one-line description in the detail pane, editable there (a new tag gets a colour from its name), one dot per tag on tree rows, a filter row of the tags in use that narrows the tree, and ⌘P matches tag names; `GET /api/v1/documents/?tag=` and `list_documents(project, tag)` do the same for scripts and Claude.
...
Today list: `list_todos`, `add_todo`, `complete_todo`, `snooze_todo`, `reorder_todos`.
