...
Claude Code can do all of it with you — 144 tools over the same API the UI uses.
...
`set_reading_status`, `browse_library`, `check_retractions`, `check_preprints`, `upgrade_preprint`, `get_new_citations`, `check_citations`, `dismiss_citations`, `get_reading_progress`, `set_reading_position`, `run_bib_check`, `get_review_matrix`, `set_review_mark`, `add_review_theme`, `suggest_review_themes`, `get_synthesis_scaffold`.
Library imports & discovery: `import_references` (BibTeX/CSL-JSON/RIS text), `import_from_zotero`,
`discover_related` (similar / cites / cited-by on OpenAlex), `export_bibtex`, `export_references` (bib / ris / csl / csv), `format_citations` (APA/MLA/Chicago/Harvard/Vancouver/IEEE).
