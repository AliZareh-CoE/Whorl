...
353. Files, the Documents table's tags and description in the explorer: the model has both, the Files tree shows neither (only the Documents page's table does) — a tag chip row and the description in the detail pane, editable there (idea added by #553)
352. A diff view for text versions in the Files history panel — line-based for `.md` / `.txt` like the notes' revision diff, a per-cell table diff for `.csv`; today a version is downloaded or restored, never compared in place (idea added by #553)
