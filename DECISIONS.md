...
## Backlog
257. Figures entry point parity: the gallery is linked from the ProjectOverview quick-links row, but the Documents page (where images are uploaded) doesn't yet point to it — a "View as gallery →" link on the Documents page header would close the loop. Also consider surfacing a figure count on the overview. Small frontend follow-on (idea added during #256)
