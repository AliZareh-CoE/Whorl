...
**Accepted behaviours.** The carry-over lives in the next card's state until Copy saves it (leaving the page drops it, like #564's presets); deleting the next step unlinks (SET_NULL) rather than blocking the delete; a chain longer than 20 hops is refused as if it were a loop; the "then" chip hands nothing over (no copy happened) — it only walks.
