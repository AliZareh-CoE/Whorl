...
## Backlog
281. Make ErrorState (#280) the SPA's default query-failure UI — wire it into the remaining data-fetching pages (Literature, Notes, Search, Graph, Writing, Reference detail) and consider a tiny `useQueryView({query, loading, error})` helper or a shared `<QueryBoundary>` so loading-skeleton + ErrorState + empty-state are consistent without per-page boilerplate (idea added by the cycle that shipped #280, 2026-06-17).
