...
370. Diagnostics: the Access section's twelve most recent events are almost always the owner's own logins, so the rows that matter (the rejected keys from Sep 15) fall out of `recent(12)` entirely — list problems first (failed logins, lockouts, rejected keys), then the last few logins, or give the section a "problems only" toggle; the #572 verdict row names the last problem, the list under it should show it (idea added by #572).
