...
**Alternatives rejected.** An in-place picker modal on each page listing the matching prompts and copying without leaving (four copies of the picker + the fill-in row; a later slice can add it on top of the address); pre-picking only the first fill-in of the kind (a prompt with two fill-ins of one kind gets the same object in both — accepted and visible in the chips); filtering by kind on the client only (Claude needs the same answer over MCP); resolving the label on the gallery from the id (a round trip for a title the source page already has; a missing label falls back to `#id`).
