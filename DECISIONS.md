...
## Backlog
282. A shared `<QueryBoundary>` (or `useQueryView`) wrapper — loading-skeleton + ErrorState are now consistent but still hand-wired per page (every page repeats the isLoading/error/!data ladder). A small wrapper taking the query result + a skeleton + a render fn would remove the boilerplate and guarantee no page ever forgets the error branch again; would also cover the still-unwired secondary pages (Graph, Timeline, Figures, Reference detail, Review, Decisions, Research, Automations, Inbox) in one place (idea added by the cycle that shipped #281, 2026-06-17).
...
279. More palette verbs (continuing #276/#277) — keep adding the safe, reversible actions the owner repeats: "Copy current project .bib", "Go to today's review", "New quick capture" (beyond the capture: prefix). Keep them side-effect-light so a mistaken Enter never destroys anything; consider a leading "›" tag or a "Commands" group header so verbs read as actions, not navigation (idea added by the cycle that shipped #277/#278, 2026-06-17).
...
275. Calm mode could go app-wide — a single body-level `.calm` class (set by the same localStorage key in the bootstrap script, like `.dark`) that other dense surfaces opt into hiding their secondary chrome (e.g. the sidebar pet bubble, the classic dashboard heatmap, figure-gallery counts). Right now #274 is SPA-dashboard-only; promoting it to a class-based variant would let CSS hide noise everywhere with zero per-component wiring (idea added by the cycle that shipped #274, 2026-06-17).
