...
- Sequence: D1 (release feed + installers) ✅ → D2 (updater) ✅. OSS-only (Tauri updater + GitHub Releases). REMAINING: owner's one-time signing setup to activate live auto-update (backlog D3).

205. Sweep the remaining empty states for an offered primary action (the audit found most already have one; the writing board was the gap — #204). A quick guard test could assert every border-dashed empty block contains an action link, but positive "all clear / inbox zero / queue clear" states are legitimately action-free, so it'd need an allowlist (idea added during #204)
...
- **210a. ✅ Desktop settings (done 2026-06-14):** config/settings/desktop.py — SQLite in ATLAS_DATA_DIR (~/.atlas), huey immediate (MemoryHuey, no Redis), WhiteNoise static, persisted SECRET_KEY, DEBUG off. Verified: `manage.py check` + full `migrate` succeed on SQLite (the whole schema is SQLite-compatible!). 2 subprocess tests guard it. Does NOT touch dev/prod.
- **210b. Search fallback:** the global search uses Postgres FTS (SearchVector/SearchQuery) → will break on SQLite. Add an icontains fallback when the DB isn't Postgres so search works in the desktop build.
- **210c. `run_desktop` command / entrypoint:** migrate + collectstatic + create-or-ensure the single superuser + serve (waitress) on a local port. One command the sidecar runs.
- **210d. PyInstaller packaging:** freeze the Django app + server into a standalone executable per OS (Linux + Windows).
- **210e. Tauri sidecar wiring:** ship the frozen server as a Tauri externalBin sidecar; main.rs spawns it on launch (ATLAS_DATA_DIR = OS app-data), waits for the port, opens the window, shuts it down on quit.
- **210f. Release CI:** extend desktop-release.yml to build the PyInstaller server per-OS before the Tauri bundle.
...
208. The doctor now has a few desktop-config checks (#203/#206); if more accrue, factor them into a small `_check_desktop()` helper so handle() stays readable (idea added during #206)
...
187. Persist filters too (status/priority on per-project literature, folder/tag on documents) the same session way, so the whole app remembers how you last looked at a list — natural follow-on to #176 (idea added during #176)
