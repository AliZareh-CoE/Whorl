...
## Backlog
264. AUDIT #24 unfinished (redo next loop): re-confirm the writable Protocol API audit and REDO the protocol_list N+1 fix that was lost to a rollback — load project.protocols once, compute the superseded parent-id set + each head's lineage in memory (template uses lineage_cached), so the page is O(1) queries instead of an is_current exists() + parent-walk per row. Add an N+1 guard test. Then write the AUDITS.md #24 entry (findings: API probes clean, ~20ms latencies, starlette was a stale-venv false alarm). (carried over from the interrupted AUDIT #24)
...
262. Protocols on the SPA Research page (follow-on to #260): the React Research.tsx page lists hypotheses/experiments/datasets read-only (writes go to the classic pages); add a read-only "Protocols" section listing current protocols (GET /protocols/?project=) with their v{n} badge, mirroring the datasets section + the "add & edit on the classic page" affordance. Also link experiments to the protocol version they followed (ExperimentEntry→Protocol FK) for provenance (idea added during #260)
261. MCP tool-list drift: the README enumerates MCP tools by hand (now 25) and the intro line still says "16 tools"; the manuscript/LaTeX + figures/calendar tools aren't all listed. A tiny guard test that the README's tool list matches the @mcp.tool() functions in server.py (or auto-generating the list) would stop it drifting. Low priority (idea added during #7-mcp)
...
258. Figure count on the project overview (deferred from #257): the overview's quick-links row links to Figures but shows no count; a small "Figures N" stat (one COUNT on documents with content_type startswith image/) would match the other at-a-glance counts. Tiny backend+frontend follow-on (idea added during #257)
