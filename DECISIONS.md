...
## Backlog
254. (AUDIT #23) The API DocumentViewSet.raw inline action sets nosniff but no Cache-Control, while document_download/document_preview both set `private, max-age=86400` on the same immutable files — so the in-app workspace PDF/image preview re-downloads every view. A one-line Cache-Control (files are immutable once uploaded) is a cheap revalidation win and a consistency fix; aligns with #11. Low risk (idea added by AUDIT #23)
