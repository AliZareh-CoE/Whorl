...
## Backlog
277. Palette verbs are a pattern worth growing — now that ⌘K can run a non-navigation verb (#276), add more of the safe, reversible ones the owner does often: "Toggle calm mode" (needs the dashboard to subscribe to the atlas-calm key first — see #278), "New quick capture" (already partly via capture:), "Copy current project .bib", "Go to today's review". Keep them reversible + side-effect-light so a mistaken Enter never destroys anything (idea added by the cycle that shipped #276, 2026-06-17).
...
278. Make calm mode live-updatable from outside the dashboard — have Dashboard subscribe to the atlas-calm localStorage key (storage event + a custom event on same-tab writes) so a future palette/sidebar toggle updates it without a remount. Prereq for the "Toggle calm mode" palette verb in #277 (idea added by the cycle that shipped #276, 2026-06-17).
