...
228. The review-page project-name chips are slug-bearing (#216) but plain text; the milestone/decision/experiment rows (plain divs, unlike the paper/note rows which are already Links) could link their project chip to the overview — partial, but those three are the ones that can without nesting a Link in a Link (idea added during #226, refines #217)
