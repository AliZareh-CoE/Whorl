...
156. Needs-attention in the SPA dashboard too — the React Dashboard island still leads with stats; port the attention lead (the API could expose needs_attention via /api/v1/weekly-review/ or a new endpoint) so both shells answer-first (idea added by cycle 145)
