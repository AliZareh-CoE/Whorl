...
369. Diagnostics: the three "Update feed" rows share one label and nothing says which address the app tries first or that the others are fallbacks — label them "Update feed · tried first / fallback / fallback", and collapse the unprobed ones into one row until the probe runs (idea added by #572).
