...
220. Now that the OpenAPI schema is 100% clean (#218/#219), a guard test could assert schema generation emits ZERO spectacular warnings/errors, so the next undocumented endpoint or unresolved type is caught at test time rather than drifting silently. drf-spectacular exposes its collected warnings via the generator (or capture the management command's stderr); the per-component asserts (#218/#219) cover the known cases but a blanket "0 warnings" guard would catch new ones (idea added during #219)
