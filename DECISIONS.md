...
260. Protocol library UI (follow-on to #7's API): a project subnav page under Research — list the current protocols (is_current), each with a "v{n}" badge + a "history" disclosure showing the version chain (parent links), an "edit as new version" form (POST new-version), and a markdown-rendered body. Classic template mirroring datasets.html, or a React Research-tab section. Also: link experiments to the protocol version they followed (a Protocol FK on ExperimentEntry) for full provenance (idea added during #7)
