...
- **D1. Ready-to-install installers (Linux + Windows).** Configure Tauri's bundler (tauri.conf bundle targets: deb + AppImage + rpm for Linux, msi + nsis for Windows) and a GitHub Actions release workflow (matrix: ubuntu + windows) that builds + bundles + uploads the installers to a GitHub Release on a version tag. README "Download" section. Can author + structurally verify (cargo check, workflow yaml lint) here; the actual binaries are produced by CI / on the owner's machine (no GUI/cross-compile in this headless container).
- **D2. In-app "Check for updates" / auto-update button.** Add tauri-plugin-updater (Rust side + capability), point it at the GitHub Releases update feed (latest.json), generate+wire the updater signing keypair (public key in tauri.conf, private key as a CI secret — owner provides), and a calm in-app control in the desktop shell: a small "Update available — restart to update" affordance + a manual "Check for updates" item. Web build hides it (desktop-only, like the terminal). Depends on D1's release feed.
- Sequence: D1 first (produces the release feed + installers), then D2 (consumes it). Keep it OSS-only (Tauri's own updater + GitHub Releases — no paid service).
