...
210. Bundle the Django server into the desktop binary (the big "NOT YET") so the installed app is fully self-contained — no separate `runserver`. Heavy (embed Python or ship a sidecar), a real epic; the owner explicitly wants to install-and-try, so this is the natural next desktop milestone after D1/D2 (idea added while answering the owner's install question)
