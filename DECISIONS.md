...
## Backlog
256. Figure-gallery UI page (follow-on to #8's data layer): a React grid under the project subnav that fetches /projects/{slug}/figures/ and renders thumbnails (img src = raw_url), grouped by folder, with a tag filter and click-to-lightbox (reuse the existing image lightbox from #227). Empty state explains "upload images to see your figures here". Frontend slice — needs the SPA route + island build (idea added during #8-data)
