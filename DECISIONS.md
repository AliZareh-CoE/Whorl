...
## Backlog
181. Folder-tree sidebar query shape — with the move-select N+1 gone, the documents index still spends ~5 queries building the folder-tree sidebar (selectors.folder_tree); worth a look at whether the nested render re-walks folders, for a tighter constant (idea added during #180)
...
183. A shared base "list page" wrapper/partial — four #25 passes (#165/#171/#179×2) repeated the same max-w-6xl + mb-4 + py-1.5 recipe by hand; a thin base template or documented snippet would make the next list surface a one-liner and keep the rhythm uniform (idea added during #179)
182. Project literature index (literature/project.html) density — the per-project Literature landing is the one literature surface #25 hasn't touched yet; give it the same treatment for full consistency (idea added during #179)
...
177. Sortable columns on the per-project literature + documents tables too — reuse the _sort_th.html partial + whitelist pattern so every list surface sorts consistently (idea added during #170)
176. Persist the library sort (session or last-used) so returning to /library/ keeps your chosen order instead of resetting to title-asc (idea added during #170)
