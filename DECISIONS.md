...
344. Today, a Trash: `TodoItem.deleted_at` with a thirty-day Trash and a `restore` action, so an undo survives a reload and a deleted done row keeps its `done_at` (the #549 client-side re-create loses it); touch every reader of todos (`open_today` / `open_later`, the viewset, `clear_done`, `spawn_next` / `unspawn`, the achievements' counts, the brief, `seed_demo`, admin) in one slice (idea added by #549).
