...
## Backlog
192. Validate ?sort= BEFORE storing it in the session (found by AUDIT #18) — #176/#190 write the raw param into session, then whitelist on read; harmless (re-validated on read) but storing arbitrary strings is untidy. Validate-before-store is cleaner defense-in-depth; natural to fold into the #191 session-remember helper (idea added by AUDIT #18)
