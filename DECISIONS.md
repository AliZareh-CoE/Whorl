...
219. The last remaining spectacular warning is an enum-name collision on "kind" (SubmissionEvent vs others share the field name with different choice sets; auto-resolved to "Kind0fbEnum"). Fix by adding ENUM_NAME_OVERRIDES to SPECTACULAR_SETTINGS mapping each kind enum to a distinct name — cosmetic (the schema is valid), low priority (idea added during #218)
