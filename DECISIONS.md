...
189. Sort/filter the per-project literature list (it's a divide-y _link_row list, not a column table) — add a small order control (status / priority / title / year), since #177 found documents already sortable but literature isn't (idea added during #177)
