...
320. Library: read every workbench filter from the URL on load (`/library?author=Lavie&tag=load`) so filtered views are linkable from notes, the brief and Claude; today only `q`, `read` and `add` are read (#524).
