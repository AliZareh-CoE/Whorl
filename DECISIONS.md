...
### 2026-09-19 — Audit #36: non-text JSON bodies, the manuscripts N+1 at real page sizes, the exemption that decorated a helper (#578)

**Decision.** The every-ten-cycles look at #569–#577: `scripts/audit.sh` clean (no advisory, no npm finding); every new surface (the two Trashes' `restore` / `empty-trash` / bulk `restore` / `purge`, the 409 guards, `?page_size=`, the Diagnostics probes, the pane editor's `PUT …/content/`) probed for auth, id bounds and junk — all bounded, nothing live broke. Three findings from reading, all fixed with tests (the report is in AUDITS.md): **a JSON body whose `content` or `note` was not a string** raised through `.encode()` (a 500) on `save_content` and `write_file` and was `str()`-ed into the note elsewhere (`"['x']"` filed as a note) — one `_text(value, field, limit)` helper in `api/views.py` answers every text body with `""` for null, a 400 naming the field for anything that is not text, and the 200-char trim for the note; **the manuscripts list was an N+1** (24 queries for 3 rows, 186 for 30 — the project, the files, the word samples, the fifteen-day revisions and the venue turnaround twice, per row) that #571's real `?page_size=200` made a page-sized problem — `select_related("project")`, prefetches for `word_samples` / `events` / `files` and a `Prefetch` of the fifteen-day `revisions` slice, `progress` and `compile_rhythm` reading the prefetched rows when present, the venue turnaround memoised per venue in the serializer context; 10 queries at 1 row and at 30, and query-budget pins now run at the sizes the pages ask for (manuscripts 3 → 30 at 200, todos 5 → 205 at 500, documents 3 → 43 at 200); **the login exemption** — `@method_decorator(login_not_required, name="dispatch")` — had drifted onto the `_pk` helper when #576 inserted `InTrash` above it, a no-op on a function that read like the guard; the API stayed exempt only because `AtlasViewSet` carries its own copy, so the stray line is gone and a test pins the exemption on the viewset base and on `SearchAPIView`, and `_pk` as an undecorated function. The #35 carry-over (the render echo's uncapped `variables`) re-checked at 20 000 chars and stays accepted. Next audit at #588.

**Why.** Two of the three are exactly what "made `?page_size=` real" (#571) and "Claude writes files" (#577) buy: a list that was fine at 50 rows is 1 200 queries at 200, and a body a person never sends is the body a mis-typed tool argument sends first. The third is why a decorator on the wrong line is worth a test: nothing failed, and the next refactor that removed the viewset's copy would have opened the API to session-less anonymous requests — the middleware would have redirected them to the login page, a soft failure that looks like a bug in the client.

**Alternatives rejected.** Validating `content` / `note` through a serializer per endpoint (four endpoints, one shape — a helper is the smaller change and the same message everywhere); dropping the manuscripts' `clock` from the list serializer to save the queries (the Writing board is the reason the clock exists); a per-request cache dict passed by hand through `progress()` (the prefetch cache is the mechanism Django already has for "rows in hand", and the functions stay callable one manuscript at a time); moving the exemption to a URL-level `login_not_required` wrap in `api/urls.py` (the viewset base is where every API class inherits from, and the test now says so).

**Post-ship review (same hour).** The advisor's read of the fix caught a hazard the suite could not: the `files` and windowed `revisions` prefetches were on the viewset's base queryset, so every detail action carried them — and `related_work`, `lint_fix`, the figure upload and `replace` write a `ManuscriptFile` whose post-save signal re-syncs the tree mirror by reading `manuscript.files.all()` on the same object; a cached set there would have left the new file out of the mirror, and a fifteen-day `revisions` cache would have shadowed the full history for any detail reader. The two prefetches now apply when `self.action == "list"` only (the project `select_related` and the pre-existing `word_samples` / `events` prefetches stay everywhere); `TestListOnlyPrefetch` pins the cache shape per action, `compile_rhythm` on both, and a file written on a detail object reaching the mirror.

**Accepted behaviours (listed, not fixed).** The render echo's raw `variables` (bounded by the request body; the stored use is capped); `?page_size=` junk falls back to the default rather than refusing; `?network=` / `?problems=` read truthiness, so `²` is "on".

### 2026-09-19 — Files: edit a text file in place with a real editor, a note, and the diff on save (#577, backlog 358)

**Decision.** The pane's textarea (file-workspace slice 2d) is replaced by a small CodeMirror editor, `frontend/src/components/CodeEditor.tsx`: line numbers, undo history, search, active line, Markdown and LaTeX highlighting by file name (everything else — .csv, .txt, .bib, .json — is plain text with line numbers), ⌘S to save (the keymap returns true so the browser's own save dialog never opens), the Observatory palette through the notes editor's exported `theme` / `highlight` and the `.md-editor` variables. It is loaded lazily when Edit is clicked, so the Files chunk stays light. The toolbar while editing: a one-line note ("What changed? (optional)", ≤ 200 chars, the version's label), Save (⌘S) / Done, Cancel, a "· unsaved" marker. The save endpoint now answers `{version, filed}` (or `unchanged: true` when the text was the same — nothing is filed), and a **Saved as v3** strip above the pane shows the diff of the filed version against now through the existing `VersionDiff` (+n −m lines, or the changed cells of a .csv from #555). The strip lives in the Files component, not the pane: a save bumps the selected file's version, which is the pane's key, so pane state would not survive. A .csv / .tsv keeps its table preview and gains **Edit as text**. Unsaved changes are guarded twice: `beforeunload` while dirty, and a confirm ("Discard the unsaved changes?") when the selection moves to another file — the state setter is wrapped (`setSelectedRaw` behind `setSelected`, same shape for values and updaters) so the twenty-odd callers stay as they were. Parity: `write_file` (the API behind `write_project_file`) answers `version` and `filed` too, so Claude can call `read_project_file(document_id, version=filed, diff=True)` and see what its write changed — the same "what changed" the pane shows. Tool count stays at 158. The shortcuts sheet gains ⌘S under Files.

**Why.** The pane already saved edits as versions, but a bare textarea (no undo history, no line numbers, no search, no highlighting) is not where anyone edits a data file or a readme; and after a save nothing said what had changed, so the version history (#553) and the diff (#555) went unused at the moment they matter most. Every editor a researcher uses (VS Code, Overleaf, Obsidian) has line numbers, undo and ⌘S; a "what changed" strip after a save is the part none of them give without a git commit.

**Alternatives considered.** The Studio's editor (`mountEditor`) — LaTeX-specific (cite completions, comment gutter, multi-file buffers) and far heavier than a text pane needs. A textarea with a note only (the note without an editor kept the seam). Autosave (a data file wants deliberate versions, not a version per keystroke; the note is the point). A three-way "Save / Discard / Cancel" on file switch (Dialog has confirm only; a two-way discard is enough — Cancel keeps editing).

**Post-ship review (same hour).** Three catches. (1) The editor's reset effect ran on every new `content` — and the file-content query refetches on window focus once it is 30 s stale, so a write from Claude or the Studio meanwhile would have replaced typed text and closed the editor without a word (the textarea had the same hole; this slice made editing the headline). A fresh `content` now never touches text the user typed: the toolbar says "changed elsewhere — Save overwrites it" (the version is kept), and only an untouched editor adopts the new text. (2) ⌘S with the cursor in the note field opened the browser's own save dialog (the keymap lived inside CodeMirror only); the note field handles it too. `beforeunload` sets `returnValue` for WebKit, the desktop webview. (3) The "Saved as v3" strip came back when a file was revisited later; it clears when the selection moves. Pins for all three in `test_pane_editor.py`. For Audit #36: the save and write-file answers now carry `filed` (an integer or null — nothing user-controlled), and `PUT …/content/` still refuses text over `TEXT_PREVIEW_CAP` (1 MB) with 413 — re-check that cap against a multi-megabyte paste from the editor.

### 2026-09-19 — Files: a Trash for deleted files — delete without a confirm, restore for thirty days (#576, backlog 357)

**Decision.** A deleted general document is no longer removed: `Document.deleted_at` (documents 0007) and a hiding default manager (`LiveDocumentManager`, built from `DocumentQuerySet` so `.general()` survives) take it out of the tree, search, every count, the dashboard, exports and reverse relations by construction; `Document.all_objects` is the one door. `documents/trash.py` carries `trash` / `restore` / `trashed` / `empty` / `prune_trash` (`TRASH_DAYS = 30`), the same shape as Today's Trash at #570. A restore lands in the folder the file left, in the project root when that folder was deleted meanwhile (the collector uses the base manager, so the trashed row's folder nulls like a live one's), and under a numbered name when a new file took its path (`write_project_file` and the importer only see live rows, so they create a twin; `available_name` + `sync_rel_path` resolve it on the way back). The delete paths all trash: the explorer (single, the selection, Del), the API (`DELETE /documents/{id}/`; `?forever=true` or a second DELETE removes for good), the bulk endpoint (`delete` trashes; `restore` and `purge` are new verbs that read the Trash — a live id is never purged, it is skipped and reported), the classic delete view, and `organize_files` (whose restore / purge resolve paths against the tree's new `trash` rows, newest deletion first). A trashed row still reads (the pane can preview it) but takes no edit — `InTrash` answers 409 on PATCH, replace, the editor save and a version restore. The nightly sweep runs from huey (`prune_file_trash_task`) and the desktop scheduler tick; `prune_media` counts a trashed file's bytes as referenced (the one silent breakage the manager change would have caused); the admin lists `deleted_at` and sees the Trash; the backup already dumps through the base manager (#570). The explorer: delete is immediate with an undo toast (single and the batch, one restore call), the confirms that remain are the irreversible ones (Delete forever, Empty the trash), and a **Trash · n · kept 30 days** section sits under the tree, outside `role="tree"` so the keys, the typeahead and ⌘P never see its rows — a struck-through name, the folder it left, "deleted today", Restore, ×. The tool count stays at 158: `list_project_files` carries `trash`, `organize_files` gains two verbs. `seed_demo` trashes one scratch file so a fresh install shows the section. Tests: `documents/tests/test_trash.py` (managers, round trip with bytes and history intact, the gone-folder and taken-path restores, the sweep unlinks, `prune_media` keeps a trashed file, bulk verbs, the API split, the classic view, the sweeps' wiring, the admin, the UI pins); the MCP client's path resolution in `test_organize_files.py`.

**Why.** Since #558 a delete unlinked the bytes on commit; the confirm dialog was the only guard, and a mis-click lost a data file with its whole history. Every product in the field (Finder, Drive, Dropbox, Notion) has a trash; a researcher expects one and expects delete to be quiet. Copying the Today shape means one mental model for "deleted" across Atlas.

**Alternatives considered.** A confirm that names the history (kept the risk, only slower). Trash for folders too (a deleted folder already moves its files to the root — nothing is lost; a folder trash would need path restoration for whole subtrees: parked as backlog 374). Trashing manuscript sources (the Studio has revisions and its own sync; a trashed source at a path the sync recreates would confuse both — kept out, the 403 stays). A global Trash page (documents belong to a project; the section under the tree is where the file was).

**Post-ship review (same hour).** Three catches. (1) The tree's `trash` rows read their folder path by walking `.parent` per row — a query per ancestor, twenty trashed files three folders deep cost forty extra queries on an endpoint the explorer refreshes constantly; the path now comes from the folders already loaded for the tree (a test pins the count constant across twenty rows). (2) A Restore into a collapsed folder looked like a vanish — the row left the Trash and nothing appeared; the explorer now reveals the restored file (its folders open, it is selected) once the refreshed tree carries it. (3) Two delete paths still described a permanent delete: the classic confirm page (now says the document goes into the Trash for thirty days, the button reads Move to the Trash) and the figure gallery (a confirm that said the image file is removed — now an immediate trash with the same undo toast; the shortcuts sheet's Del line says the same). For Audit #36: the `@method_decorator(login_not_required, name="dispatch")` above `_pk` in `api/views.py` is a no-op on a plain function (it only takes effect on a class) — it predates this slice and is left as found; the audit should decide whether it belongs on a view.

### 2026-09-19 — Diagnostics: the last seams, and the Diagnostics area verdict (#575)

**Decision.** One more stranger-read of the page, measured first. (1) At a 640-px window the header actions row (the probe checkbox, Download a backup, the last-backup stamp, Copy report) ran 16 px past the column; at 420 it "fit" only because the labels broke mid-phrase ("Probe / the / update / feed" on four lines). The row now wraps as units (`flex-wrap` + `whitespace-nowrap`) — `scrollWidth === clientWidth` at 640 and 420, the buttons whole. (2) The inherited feed-tick rule went green on `status 200 && key_match !== false`, so an unsigned feed (`key_match` null) showed a green tick beside a verdict row saying "unsigned"; a row is green only for a feed signed for this app, no icon for unsigned, red for a wrong key or a bad answer — test-covered (no live state on this box exercises it). (3) The report has carried `disk` since #572 but the page said nothing about free space above the warn threshold and the text never printed it — a reader pasting a report for "the upload failed" with 1.5 GB free got nothing; a **Disk** row under Data folder ("23.8 GB free of 270 GB at <path>", red under 1 GB) and a `disk:` line in the text. (4) A finding's title and detail were two spans with visual spacing only, so a copy from the page ran them together; a real space now sits between them.

**Verdict — Diagnostics is best-in-field for a single-user research tool.** A stranger with a broken install opens one page and reads, in order: a one-line verdict and every finding with its evidence, its fix and a link (#572, nineteen rules, fail before warn, a fallback verdict that can never take the page down); what this install is (version, system, data folder, database, disk, LaTeX engine and TeX bundle with a one-click warm-up, jobs, API key, embeddability); whether it can update — the feed chain labelled tried-first / fallback with the address the app actually used marked and only that one able to go red (#574, #534); who touched the door — problems first, the last logins apart, the raw tail on request (#573, #399); front-end errors and the last failed compile's log; backups — restore from a file or a snapshot, automatic snapshots, an attached drive or sync folder as a destination (#376, #462, #463, #536); the server log tail; and one **Copy report** whose text leads with the same verdict, so the paste answers "why didn't it work" before the facts. Everything on the page is in `GET /api/v1/diagnostics/` and in `get_diagnostics` for Claude, and `manage.py doctor` gives the shell-side view. Against the field: no research tool (Zotero, Paperpile, Obsidian, Overleaf) ships a self-diagnosis page at all; the nearest comparison is an IDE's "Report a problem" bundle, which collects but does not judge.

**What a stranger with a broken install would still miss** (logged, not built this hour): (a) the doctor's shell-side checks — Redis reachable, the worker alive and running current code, migrations applied — are invisible to the page because a request cannot see them; on the desktop (the primary install) none of them apply, on a server they are the checks a stranger most needs, so a page-side "run the doctor" that spawns the command and streams its lines is the sharpest miss (backlog 371; 252 owns the doctor refactor that would make it one code path); (b) a finding's age — whether "no recent backup" appeared this morning or has sat there for a month — needs a small store of first-seen stamps (backlog 372); (c) the report as a downloadable file next to Copy (a paste is fine for a chat, a file is what a bug tracker wants — backlog 373).

**Post-ship review (same hour).** The new Disk row disagreed with the verdict block on the same fact: it went red under 1 GB while `findings()` calls 200 MB – 1 GB a warn (amber, "nearly full") and only under 200 MB a fail — at 900 MB free the page would have shown an amber warning on top and a red cross below it, the icon-versus-verdict mismatch this very slice fixed on the feed row. The row is red only under `NO_DISK_BYTES` now, no icon otherwise, and the wiring test pins the TSX literal to the Python constant so the two cannot drift. The pointer line in `cycle.md` also carried an unclosed parenthetical from the move; folded.

**Alternatives rejected.** Hiding the actions behind a menu below 640 (four buttons wrap cleanly onto two rows; a menu hides "Copy report", the one action the page exists for); showing the disk as a finding only (the page should say what is fine, not only what is wrong); folding the doctor into the page this hour (a subprocess from a request needs the streaming and the guard rails of a slice of its own).

**Evidence.** `core/tests/test_diagnostics.py` (the three-way tick rule pinned), `core/tests/test_findings.py` (the Disk row and the wrapping actions row pinned; the `disk:` text line with a value and with `unknown`). Overflow probe at 640 and 420: `scrollWidth === clientWidth` at both, no element past the viewport; the 420 screenshot shows the four actions whole on two rows. Playwright both themes at 1440: the Disk row reads "22.1 GB free of 252 GB at …/media" with no icon. Desktop runs 310/311 green.

### 2026-09-19 — Diagnostics: the update-feed rows say which address the app tries first, and only the one it uses can go red (#574, backlog 369)

**Finding.** Three rows labelled "Update feed" listed three addresses with nothing saying the app tries them in order, that the second and third are fallbacks, or which one actually answered. After a probe the third row (`atlas-releases`, 404) wore a red cross although the first address had answered and the app never consulted it — a red mark on a page whose verdict said everything was fine.

**Decision.** `update_feed_status()` rows carry `role` ("first" for index 0, "fallback" otherwise — derived from position, the order the app uses); `update_verdict()` carries `url`, the one address whose answer decided the verdict (`feeds[0]`, the first endpoint that answered with a feed), `None` for unchecked / offline / unreachable. The page: until the probe runs, one row — "Update feeds: <first address> · tried first · + 2 fallbacks · project-manager, atlas-releases" (the probe checkbox is the expansion; no new state); after it, one row per address labelled "Feed · tried first" / "Feed · fallback" (short enough for the label column), a chip "the app uses this one" on the address that matched `verdict.url`, and the icon rule: the used row keeps the old rule (red only for a wrong key or a bad answer), rows after it get no icon and the suffix "not consulted — an earlier address answered", rows before it that failed show their status without red (the chain worked as designed), and when nothing answered every probed row keeps the old rule because every failure is real. `as_text` labels each line "(tried first)" / "(fallback)" and marks the used one "← the app uses this one" — full URLs stay, a bug report wants them. The doctor prints the verdict text and is unchanged; the MCP docstring is unchanged (the budget has no headroom; `role` and `url` ride along in the payload).

**Post-ship review (same hour).** The rule had a gap symmetric to the one it fixed: a failed address *before* the one the app used (the primary dead after a rename, the mirror carrying updates — the very install the fallback chain exists for) rendered as a bare "→ 404" with no icon and no words, directly above a row saying "the app uses this one". It now reads "· no feed here — the app moved on to the next address", on the page and in the text; the text test's fallback-answers case asserts it (this state cannot be probed live — the first address answers on this box — so it is test-covered). The collapsed row's muted span gets `break-normal` so the fallback names wrap at spaces, not mid-word. Inherited and left alone: the used row goes green on `status 200 && key_match !== false`, so an unsigned feed shows a green tick beside a verdict that says "unsigned" — the verdict is the authority; #575's stranger-read decides whether the row should demand `key_match === true`.

**Alternatives rejected.** A `used` flag on each row (a side effect of the verdict on the rows; the verdict owning `url` keeps the rows pure); a toggle to expand the collapsed row (the probe is the expansion — there is nothing to see about unprobed addresses beyond their order); hiding the fallback rows after a probe (a reader asking "does the mirror work?" wants to see it answered).

**Evidence.** `core/tests/test_diagnostics.py` (roles by position; `verdict.url` for a first-answers case, a fallback-answers case — Whorl 404, project-manager 200 → project-manager — and `None` for unchecked, unreachable and offline; the text labels and marker; UI wiring incl. the icon rule and the negative pin that the bare rows are gone). Playwright both themes at 1440: collapsed row before the probe (0 feed rows); after "Probe the update feed" — Whorl `tried first · used · green · 200 · offers 0.1.309 · signed for this app`, project-manager `fallback · no icon · 200 · not consulted`, atlas-releases `fallback · no icon · 404 · not consulted` — the before/after this slice exists for: that last row was a red cross at #573. The verdict on this box stays `unknown_version` (a source install reports "dev"); the schema passes `--fail-on-warn`.

### 2026-09-19 — Diagnostics: the Access section lists the problems first (#573, backlog 370)

**Finding.** The section showed the twelve most recent access events. On any install the owner's own logins are the bulk of those, so the rows that matter — the nineteen rejected keys from Sep 15 on this box — had fallen out of the list entirely; the verdict row from #572 said "the list below names it" and the list did not.

**Decision.** `core/access.py::problems(days=7, limit=12)` — failed logins, lockouts and rejected keys in the window, newest first — and `recent_logins(limit=5)`, both through one `_row` shaper; `recent()` keeps its meaning (the `/access-events/` contract is pinned). The report's `access` carries `problems`, `logins` and the unchanged `events` tail. The page: **Problems · last 7 days** first (amber chips, lockouts in red — the one kind that means someone kept trying; address, agent and detail on every row; "n more in the window" when capped, from the counts the summary already carries), a calm empty line otherwise ("None in the last 7 days — no failed login, no lockout, no rejected key. The last one was on <date> from <address>." when `last_problem` predates the window — that field is not windowed and the mismatch is handled in words rather than a new field); then **Recent logins · last 5**; then a "Show the last 12 events of every kind" toggle over the raw tail (its state hook sits with the others, above the loading return). `as_text` prints the problem rows under the counts line — the pasted bug report now carries what the verdict points at. `GET /access-events/?problems=1` lists only the problems (documented for the schema); `get_diagnostics` carries the new keys with its docstring unchanged (the core budget has no headroom).

**Post-ship review (same hour).** The "n more" line claimed "the rest are the same kinds" — untrue by construction: the cap is newest-first, so under a probe storm the older lockout is exactly what falls off the end, and the verdict row's "the list below names it" would have pointed at a list that did not. `core/diagnostics.py::hidden_problems(counts, shown)` now names what the cap left out per kind (the window's counts minus the rows shown — "5 failed logins · 1 lockout"), on the page and in the pasted text; pinned with a synthetic 12-rejected + 1-lockout report. `core/access.py::WINDOW_DAYS` is now the one default for `summary()` and `problems()`, so the section's "last 7 days" label is honest by construction (pinned). Noted for Audit #36: `_access()` went from three queries to five on a table pruned to 500 rows; no budget pins the diagnostics endpoint.

**Alternatives rejected.** Filtering `events` on the client (under an actual attack the tail is all rejections and the logins vanish with no explanation); a "problems only" toggle over one list (the reader should never have to ask); a new `problems_total` field (the counts already give it); windowing `summary.last_problem` (the verdict row wants the last one ever).

**Evidence.** `core/tests/test_access_problems.py` (windowed, capped, newest first, logins never crowd them out and `recent()` unchanged; `recent_logins` is logins only; the report carries the three lists; the text names a problem row under the counts line; the endpoint's `?problems=1`; a malformed row is skipped; UI wiring pins including the order problems → logins → tail). Playwright both themes at 1440 on this box: 12 amber `api_key_rejected` rows, "7 more in the window", 5 logins, the raw tail hidden until the toggle and 12 rows after it — the honest state; the empty-state wording is test-covered. `?problems=1` and the schema pass `--fail-on-warn`.

### 2026-09-19 — Diagnostics leads with a verdict: what is wrong, and the fix for each (#572)

**Finding.** Read as a stranger with a broken install, the page was a list of facts — sixteen rows of "This install", the access log, snapshots, a log tail — with green ticks on some rows and no sentence anywhere saying whether anything is wrong. The question the page exists for ("why didn't it work?") was left to the reader.

**Decision.** `core/diagnostics.py::findings(report)` — a pure function over the collected report, no I/O, every optional section may be `None` — turns the facts into rows `{id, level, title, detail, fix, link}` with two levels only: **fail** (broken now: no LaTeX engine, a failed warm-up, no API key, the files folder not writable, the disk full, a failed snapshot, the backup destination unreachable or its last copy failed) before **warn** (will bite or is drifting: a cold TeX bundle, the disk nearly full, the desktop scheduler not running, the destination behind, a stale or missing backup, the last compile failed, front-end errors recorded, requests with a wrong API key, failed logins or lockouts, an updater that cannot update or could not be reached, a traceback in the server log). `verdict(rows)` gives one sentence — "Nothing wrong that Atlas can see." / "1 thing broken · 2 to watch." Two new report keys feed it: `disk` (free space where Atlas writes, `shutil.disk_usage`, thresholds 1 GB / 200 MB) and `media_writable` (`os.access`, not a probe file — this runs on every load). The page leads with the verdict block (colour-edged, one row per finding with its fix and an Open link where a route exists: the Studio for a failed compile, the Connect page for the key), the pasted `text` leads with it, and `get_diagnostics` names it first. Noise gates: the updater rules fire only when the feed was probed and `offline` is a warn at most; the rejected-keys row names the last stamp and address and says "a client with a stale key — or something probing" so the reader can judge rather than be alarmed; the backup rules are gated on `has_data`. The doctor command keeps its own inline checks (backlog 252 notes the overlap).

**Post-ship review (same hour).** Three catches. `collect()` called `findings()` bare while every other section is wrapped "must not break the page" — a malformed cache entry (a front-end error record that is not a dict) would have 500'd the one page that must render when things are broken; the call is now guarded and lands as a fallback verdict ("The verdict could not be computed — copy the report anyway."), pinned by a test that feeds the rule a junk entry and asserts 200 — and that test found a second hole on the same path: `as_text` indexed into the same entries, so the text renderer now skips anything that is not a dict. The ok-state copy claimed "the backups are fresh" on installs where the backup rules are skipped (`has_data` false) — it now says "Nothing needs doing." and the wiring test forbids the old claim. Two rules were noisy: a single mistyped password warned for seven days (now a lockout, or three misses, `FAILED_LOGINS`), and the most recent failed compile warned forever (now within `COMPILE_DAYS` = 7, from the manuscript's stamp; a missing or junk stamp still counts).

**Alternatives rejected.** A new collector with its own probes (the report already carries the facts; one function over it stays testable with synthetic dicts); an "info" level (two levels keep the block honest — a row is there because something needs doing); folding the doctor into it (the doctor is an ops tool with Redis and migration checks that need a shell; 252 owns that refactor).

**Evidence.** `core/tests/test_findings.py` (a healthy report yields no findings and a green verdict; every optional section may be missing; one test per rule with its level; the quiet states — running warm-up, current updater, fresh backup, destination off, live scheduler — fire nothing; details carry the stamp, the address and the links; fails sort before warns and the verdict counts them; the text leads with the verdict; the live API carries `verdict`, `findings`, `disk`, `media_writable`; UI wiring pins). Playwright both themes at 1440: the block sits above "This install", state `warn`, one finding `access_rejected` ("19 requests carried a wrong API key · In the last 7 days (last 2026-09-15 17:56 from 127.0.0.1)"), its Open link lands on /connect — that is the real state of this box (the 19 are Audit probes), so the amber rendering is the honest screenshot; the fail and all-clear renderings are covered by the tests. MCP core budget kept under 5200 by trimming `get_diagnostics` to 263 chars.

### 2026-09-19 — Every list was capped at 50: `?page_size=` made real, and the Logbook paged (#571, backlog 345)

**Finding.** Backlog 345 said the Today page's fetch cap of 200 was the tell. The tell was wrong by a factor of four: `REST_FRAMEWORK["DEFAULT_PAGINATION_CLASS"]` was DRF's stock `PageNumberPagination`, whose `page_size_query_param` is `None`, so **every `?page_size=N` the SPA has ever sent was silently ignored** and every list came back at the default 50 — the Report page asked for 500 references and got 50, Notes / Inbox / Writing / feeds / highlights asked for 200 and got 50, the Today page and the sidebar nudge asked for 200 and got 50. Reproduced before the fix with 61 seeded done rows: `GET /todos/?page_size=200` → `count 69, results 50, next: true`.

**Decision.** `api/pagination.py::AtlasPagination` — `page_size_query_param = "page_size"`, `max_page_size = 500` (the largest a page asks for), the default unchanged at 50 — is now the default paginator; junk falls back to 50, anything over the cap gets the cap. drf-spectacular now emits the `page_size` parameter on all 33 list endpoints. After the fix the same request → `results 69`. No other page changes: they already asked for the right sizes and now get them. For Today, two `when` values on `/todos/`: **`when=logbook`** (`logbook_q()`, ordered `done_at DESC NULLS LAST, -id` so Postgres and SQLite agree and an unstamped row lands last; paged 50 at a time) and **`when=current`** (everything but the Logbook — open items and today's ticks — the one fetch the page shows above it). The page fetches `when=current&page_size=500` under `["todos", "current"]`, the Logbook through `useInfiniteQuery` under `["todos", "logbook"]` with a **"Show older · n more"** row and the count in the header; "since …" appears only once the whole record is in (the oldest *stamped* row, so an undo's unstamped row never blanks it); the nudge fetches open rows only under `["todos", "open"]`. Every exact-key read and write of the main query moved with it (twelve sites) — the prefix invalidation `["todos"]` in refresh, the command bar, the dashboard and the inbox reaches all three. `list_todos(when="logbook")` for Claude; the client no longer adds `done=false` for it (the Logbook is done rows by definition — caught live: the first call returned 0 rows).

**Post-ship review (same hour).** The advisor caught a regression the probes had not reached: a delete from the Logbook lost its undo — `remove.onMutate` looked the row up in `["todos", "current"]` alone, Logbook rows no longer live there, so `gone` was null and the "in the Trash for 30 days" toast (and `z`) never came. Fixed by reading both caches (`InfiniteData` pages of `["todos", "logbook"]` flattened next to the live rows); pinned in `test_ui_wiring`; the probe now deletes a Logbook row, sees the toast, presses `z` and finds the row back in the Logbook and out of the Trash, both themes. Also noted for Audit #36: every SPA list now really receives what it asks for, so the query-budget pins taken at 50 rows should be re-run at the sizes the pages request (the manuscript list with `clock` per row first).

**Retention: none.** The Logbook is the record — the owner's list is "never lost overnight" — so the nightly-prune half of 345 is rejected; paging is the answer that keeps every tick, and "Clear the logbook" (into the Trash since #570) stays the one manual sweep.

**Alternatives rejected.** Raising `PAGE_SIZE` to 200 globally (every list pays for the Report's appetite); a separate `/todos/logbook/` endpoint (a `when` value keeps one list, one schema entry, one MCP tool); fetching the Logbook in full (the cap it was born to avoid); a "since" from the first page's last row (it would name the wrong day until the last page).

**Evidence.** `core/tests/test_logbook_pages.py` (the paginator is the default and honours 60 > 50, junk and over-cap fall back, `when=logbook` pages 50 + 11 newest first with the unstamped row last and today / open rows out, `when=current` is everything but the Logbook, the Trash stays out of both, UI wiring pins including `Today.tsx` naming `["todos"]` exactly once); `mcp_server/tests/test_client.py::test_todo_client_calls` (logbook without `done=false`); Playwright both themes at 1440 with 61 seeded rows: header "LOGBOOK · 62", 50 rows + "Show older · 12 more", after the click 62 rows, 11 day groups, the unstamped row last, header "LOGBOOK · 61 · since Wed, Sep 9"; a tick from the list lands in Done today and `z` takes it back with the Logbook untouched; an untick from the Logbook returns the row to the list and the count drops by one. Live curl: `page_size=200` → 69/69, `page_size=9999` → 69 (cap 500), `page_size=abc` → 50. Desktop runs 302 and 303 (the #570 pair) green.

### 2026-09-19 — Today: a Trash — a deleted item waits thirty days, comes back as it was, keeps its chain (#570, backlog 344)

**Decision.** `TodoItem.deleted_at` (core 0016, indexed) and two managers: **`objects` hides the Trash** — it is the first-declared manager, so every reverse relation (`project.todo_items`, `item.repeats`), `open_today` / `open_later`, the brief, the dashboard hero, the nudge, the achievements' done counts, `reorder`'s known-ids check, the capture triage and the seed's lookups stop seeing a deleted row *by construction*, with no edit; `all_objects` is the one door to the Trash (the viewset's class queryset, the admin, the sweep). `DELETE /todos/{id}/` now **trashes** (`core/todos.py::trash`, a `save()` so the ETag moves); `?forever=true`, or a second DELETE on a trashed row, deletes for good; `POST /todos/{id}/restore/` brings it back exactly as it was — its day, its rule, its done stamp, its position (idempotent); `?trash=true` lists the Trash newest first; `POST /todos/empty-trash/` clears it; a PATCH or snooze on a trashed row is a 409 ("restore it first"). **Clear done trashes too** (`deleted` keeps its meaning, `trashed` rides along), so the Logbook's clear is recoverable for thirty days. **The chain invariant holds through the Trash:** `spawn_next` looks at every successor, trashed or live — a re-tick of a parent whose open successor sits in the Trash *restores* that successor rather than spawning a second one; `unspawn` on an untick removes an untouched successor whether live or trashed (the occurrence never happened); an edited successor in the Trash stays there and comes back on the re-tick. The sweep — `core/tasks.py::prune_todo_trash_task` nightly at 05:40 under huey, and one more block in the desktop scheduler thread — hard-deletes rows past `TRASH_DAYS` = 30 (a pruned parent detaches its successors by `SET_NULL`, as clearing done rows always did). The page: the delete undo now calls `restore` (no client-side re-create, no second tick, no reorder splice — the #549 workaround is gone), and a **Trash** section under the Logbook (collapsible, remembered) lists deleted rows with the day they went, Restore, a confirmed Delete forever, and a confirmed Empty the trash. Claude: `list_todos(when="trash")` and `trash_todo(todo_id, restore, forever)` — the count stays at **158** because `get_compile_diagnostics` folded into `get_compile_status` (its `diagnostics` key was the whole tool). `seed_demo` puts one row in the Trash three days ago. Migration 0016; `manage.py migrate` on an existing install leaves every row live.

**Why.** Backlog 344 named the three costs of the #549 client-side re-create: an undo that did not survive a reload, a done row that lost its stamp, and a re-created successor that broke the one-open-occurrence rule. All three come from the same root — the row was gone — and the honest fix is that a delete is not a delete for thirty days, which is what Things, Todoist and the Finder do. A default manager that hides the Trash is the only design where "one missed reader leaks a count" cannot happen: the readers were never changed.

**Alternatives rejected.** A `deleted` boolean with every reader filtering it (the leak the backlog item warned about); keeping `objects` as everything and filtering in the viewset only (the brief, the achievements and `item.repeats` would count trashed rows); hard-deleting on the second DELETE without a confirm in the UI (the API is explicit, the page confirms); an undo that restores through POST + PATCH as before (the stamp and the chain are exactly what the re-create could not carry); a trash count fetched separately (one small query alongside the list is cheaper than a count endpoint); container-scoped rows for the Trash section (the LogRow layout and #551's wrap rules carry over unchanged).

**Post-ship review (same hour).** Two readers of `_default_manager` the tests had not reached: the JSON backup (`dumpdata` on a non-SQLite install) would have dropped the Trash — it now dumps through the base manager (`use_base_manager=True`; the SQLite path copies the file and was never affected); and the inbox's "Recently triaged" resolver looked a converted todo up through the live manager, so a trashed one read as missing — it reads `all_objects` now. Both pinned in `test_trash.py`. The list ETag was checked live: clear-done's bulk update moves `updated_at`, so the next conditional GET is a 200. The shortcuts sheet's `x` row says "into the Trash for 30 days".

**Accepted behaviours.** The seed's live `get_or_create` re-creates a demo row the owner trashed (it always did after a hard delete); a restored row keeps its old `position`, which may tie with a newer row's (the list orders by position then id); `trash_todo(forever=True)` answers `{deleted: true}` without a row body (the row is gone).

### 2026-09-19 — Prompts: the narrow-width pass, and the Prompts area verdict (#569)

**Decision.** Measured before anything was changed: at a 640-px window (a 336-px column beside the rail) the gallery scrolled 193 px sideways — every card's header row (title, tags, the "then" chip, "used n×", Copy, ⋮) was one non-wrapping line, and the toolbar's 18-rem search box plus "n shown · New prompt" ran 32 px past the column; at 420 (no rail) the cards still overran by 157 px. Four changes in `Prompts.tsx` — no new stylesheet utilities, `make css` was a no-op. (1) The card header is **title · actions · meta**: the title never forces a wrap (`flex-1 basis-0`), grows only to its own width (`max-w-max`, so on a wide screen the chips still sit right after it) and truncates; Copy, the "Next: …" step and ⋮ are one atomic group placed *before* the chips in the DOM, so below `lg` the buttons stay on the title line and the chips (tags, "then → …", "used n×") drop under it on a line of their own (`basis-full`); from `lg` the chips sit beside the title and the actions go last (`lg:order-last`) — the #551 rule, flex wraps in DOM order, so the group that must stay on the first line comes first. (2) The toolbar wraps; the search box gives way (`flex-1 basis-40 max-w-72`) before the count and the button do. (3) The form's two columns start at `lg` instead of `sm`, and the "then" row wraps. (4) The picker: a picked chip shrinks inside its label instead of holding its 20-rem cap past the column, and the hit list — 20 rem wide, capped at the viewport — hangs from the input's *right* edge when its left-anchored box would run past the viewport (a `useLayoutEffect` measures it once per open). Measured after: 640 / 420 / 1440 in dark and 640 in light, with the history panel, the picker, the "Use a prompt with this…" banner and the form open — `scrollWidth === clientWidth` everywhere; the desktop title box is pixel-identical before and after. Measured with the Next step offered too (the post-ship review's case): after a Copy the "Next: Find the gap in {{paper}} →" button joined the actions group and ran 16 px past the card at 640 and squeezed the title to 3 px at 420 — below `lg` the button now reads "Next →" with the title in its tooltip, unchanged from `lg` up. Accepted: a picker list that flipped to the right edge stays flipped until the card remounts (widening the window does not un-flip it).

**Verdict — Prompts is best-in-field for a single-user research tool.** After #562–#569 a prompt is a small program over the researcher's own data: `{{paper:reference}}` / `{{note:note}}` / `{{project:project}}` / `{{draft:manuscript}}` are **picked from Atlas** and expand to the row's title, authors, venue and abstract at copy time (the server renders, so Claude's `get_prompt(values)` copies the same text); `{{venue|NeurIPS}}` defaults and remembered fill-ins; every copy is a **use** — a count, a Recent strip, a per-prompt **history** with "Use again"; **"Use a prompt with this…"** on every kind of object (a paper's kebab and the Library pane, a note's header, a manuscript's kebab and the Studio palette, the project overview) opens the gallery filtered and pre-filled; **chains** (summarize → find the gap) offer the next step after a copy with the fill-ins carried over, with a loop guard in the API and the admin; search, tags, a right-click menu, an empty state that writes the first prompt for you, and the page works down to a 420-px window; Claude reads and renders the same gallery (`list_prompts(kind)`, `get_prompt(values, history)` — 158 tools, flat since #562). Judged against Raycast (snippets and AI commands: dynamic placeholders, a hotkey), Notion AI (inline invocation on the object you are looking at) and PromptLayer (template versioning, per-template analytics): Atlas has Notion's "on this object" entry point on every object, PromptLayer's use analytics per prompt, and a placeholder Raycast cannot have — one typed from the researcher's own library. What a user of those would still miss, checked against the code: **versions of a prompt's body with a diff** (an edit overwrites; the notes' revision model would fit — backlog 366); **a ⌘K "Copy a prompt…" verb and a global hotkey** (the palette creates and navigates; copying a prompt means the gallery page — 367); **import / export of the gallery** (prompts are rows in the backup and the API; a colleague gets them by JSON over the API, not by a file — 368); **more fill-in kinds and dynamic placeholders** (`{{file:document}}`, `{{today}}`, `{{clipboard}}` — 364); **clear history** (365); and **running the prompt in-app** — by owner rule (no paid LLM APIs) the runner is Claude over MCP, which is why the gallery is an API contract first. None of these is what a researcher hits daily at the copy button; 366 is the first candidate for a return pass. The pointer moves to **Today's return pass** — backlog 344 (a Trash) at #570 and 345 (Logbook retention) at #571 — then Diagnostics; Audit #36 stays due at #578.

**Alternatives rejected.** A container query on the card (`@container` / `@lg:`) as on the Today rows at #551 — the kebab's menu is `position: fixed` from viewport coordinates and not portaled, and `container-type: inline-size` applies layout containment, which makes the card the containing block for fixed descendants: every ⋮ menu would open offset by the card's position; viewport breakpoints leave the menu alone, and the rail becoming a drawer below 640 makes `lg` (a 720-px column from 1024) the honest threshold for "chips beside the title"; a two-row header at every width (the desktop row was fine and the screenshot proves it unchanged); `justify-between` on the wrapped toolbar (a lone second-line item lands on the left; `ml-auto` on the group keeps it right); a left-only hit list capped at the viewport (the cap ignores the input's offset — it still ran 12 px past at 640); hiding the chips below `lg` (they carry the chain and the count).

### 2026-09-19 — Audit #35: an anyio advisory, a Unicode-digit 500, a repr leak, the admin loop (#568)

**Decision.** The every-ten-cycles look at #559–#567: `scripts/audit.sh` flagged **anyio 4.13.0** (CVE-2026-63374 IDNA 2003 host names in `TLSStream`; CVE-2026-64847 process-pool workers blocking on stderr) — bumped to 4.15.1 through the lock (`uv lock --upgrade-package anyio`; the desktop syncs from the same lock and `atlas_mcp.spec` already bundles anyio), pinned ≥ 4.14.2 by a test. Every new surface (render values, `?kind=`, `/uses/`, `next`, tag merge / rename / colour, bulk verbs) probed for auth, bounds and junk — three findings, all fixed with tests (the report is in AUDITS.md): **a Unicode digit** (`"²"`) as a typed fill-in was a 500 because `isdigit()` guarded an `int()` that refuses it — `isdecimal()` is the exact predicate and replaces `isdigit()` at the ten `isdigit → int` pairs in the code; **an object or a list** as a render value was `str()`-ed into the text as a Python repr — refused with a 400 whose message never starts with "No " (the 404 shape), and a refused render is not a use; **the admin** could save a chain loop — `PromptAdminForm.clean()` runs the same `chain_would_cycle` as the API. The #34 carry-over closes: the zip copies members through `shutil.copyfileobj` and Duplicate streams the open `FieldFile`, so no path reads a file whole. Next audit at #578.

**Why.** An audit is where "it worked on every value we typed" meets the values nobody types: `str.isdigit()` is the textbook Python trap (True for `²`, `①`, `½`-adjacent digits that `int()` rejects) and it guarded ten `int()` calls, one of them reachable from Claude's `get_prompt(values=…)` with any string; an object in a JSON body is the next thing a client sends by accident. Fixing the predicate everywhere in one sweep is cheaper than meeting each site at its own 500. The anyio bump is routine but the routine only works when it is run every ten cycles and acted on the same hour.

**Alternatives rejected.** `try: int(value) except ValueError` at each site (works, but leaves `isdigit()` reading as if it were the guard — `isdecimal()` says what is meant); pinning anyio in `pyproject.toml` (it is transitive; the lock is the pin and `pip-audit` reads the lock); `str()`-ing objects with `json.dumps` instead of refusing (a prompt value that is an object is a client bug, and the text would carry JSON nobody asked for); fixing the admin by hiding `next` from the form (the back office should be able to set a chain, just not a loop).

**Accepted behaviours (listed, not fixed).** The render response echoes the raw `value` / `label` uncapped (bounded by the request body; the stored use is capped at 300); a negative or non-numeric typed value is used as text (the documented Claude path); `?kind=` junk is ignored rather than refused.

### 2026-09-19 — Prompts slice 6: chains — a prompt names the one that comes next, and hands its fill-ins over (#567)

**Decision.** `Prompt.next` (prompts 0004; a self FK, `SET_NULL`, many prompts may lead to the same next step). Atlas never runs a model, so the honest chain is "after you copy A, B is next, and what you picked for A is already filled into B": after Copy the card shows **Next: Find the gap →**, which scrolls to and flashes the next card with the fill-ins carried over — by the same variable name first, else (for a typed fill-in) the first value of the same kind; a text fill-in without a namesake stays empty. A "then → Title" chip on every card with a next step walks the chain without copying. The edit form gets a "then" select over the other prompts. `prompts/services.py::chain_would_cycle` walks up to 20 hops and the serializer refuses a loop (self, 2-cycle, longer) with a 400; the API carries `next` (writable) and `next_title`; the list `select_related`s the next step so the ≤ 8-query budget holds. MCP `get_prompt` / `list_prompts` carry both fields for free — tool count flat at 158. `seed_demo` adds "Find the gap in {{paper}}" and chains Summarize → Find the gap, only while Summarize has no next step.

**Why.** A literature pass is a sequence — summarize, find the gap, draft the critique — and today each step means finding the next prompt and re-picking the paper. Raycast and Notion AI have no notion of one prompt following another; PromptLayer chains are code. A pointer plus a carry-over is the whole feature for a tool that copies text: the researcher runs the chain in their own model, Atlas keeps the object in hand.

**Alternatives rejected.** Prompt inclusion — `{{step:prompt}}` expanding another prompt's body into this one (nests fill-ins, hides what is copied, and answers a different question: composing text, not sequencing work); a separate `Chain` model with ordered steps (a linked list on the prompt is the same data with no second screen; a chain is read by following `next`); a many-to-many "next steps" (one next keeps the card's affordance a single button; a fork is two prompts); carrying every value blindly (a `{{venue}}` on B is not A's `{{topic}}` — names first, then kinds).

**Accepted behaviours.** The carry-over lives in the next card's state until Copy saves it (leaving the page drops it, like #564's presets); deleting the next step unlinks (SET_NULL) rather than blocking the delete; a chain longer than 20 hops is refused as if it were a loop; the "then" chip hands nothing over (no copy happened) — it only walks; the Next button is offered from the copy until the card's fill-ins change (not for the two seconds the "✓ Copied" flash lasts — a researcher pastes elsewhere and comes back); the loop guard lives in the serializer, so the Django admin can still save a loop (harmless: the guard's visited set terminates, the gallery follows one hop, MCP only reads — listed for Audit #35); a handoff into a card the current filter hides is kept and applies when that card next renders.

### 2026-09-19 — Prompts slice 5: every kind of object has its "Use a prompt with this…" — the project overview, the Library pane, the Studio (#566)

**Decision.** The three entry points #564 left out: the project overview's menu ("Use a prompt with this project…" → `/prompts?use=project:<id>&label=`; the overview payload already carried the project's id through `ProjectSerializer`, only the SPA type omitted it), a **Use a prompt** link beside *Open* in the Library's detail pane (the row menu of #564 stays; the pane is the second path, for the paper you are already reading), and a Studio palette action ("Use a prompt with this manuscript…", ⌘⇧P) that **saves first** and then leaves for the gallery, so a dirty buffer is never dropped. `seed_demo` gains a `{{project:project}}` prompt ("Progress update on {{project}}") so the project kind lands on a hit like the other three. No API or MCP change — tool count flat at 158.

**Why.** #564's own probe showed the `project` kind had the filter and the empty state but no way to arrive; a menu item on the overview closes that. The Library pane is where a paper is read, so the link belongs next to Open. The Studio is a full-screen editor: a palette action is its menu.

**Alternatives rejected.** A ⌘K verb "Use a prompt with…" (it would need a picker for the object — the pages already know the object); a "Use a prompt" button in the Studio top bar (the bar is full; the palette is discoverable via ⌘⇧P and searchable); navigating from the Studio without saving (an autosave may be pending; the action calls the same save the ⌘S binding does).

**Accepted behaviours.** A paper now has three paths (the Library row menu, the Library pane link, the Reference page menu) — intentional, one per place a paper is met; leaving the Studio for the gallery has no "back to the editor" affordance beyond the browser's back (the manuscript page's "Open the studio" is one click away); a `{{project:project}}` expands to the project's name and description only — not its plan, status or papers (the status update of #482 is the richer text, reachable from the same menu).

### 2026-09-19 — Prompts slice 4: a history per prompt — what it was copied with, and "Use again" (#565)

**Decision.** Every copy now files a `PromptUse` row (prompts 0003): the resolved fill-ins of that render — name, kind, default, value, label — i.e. the picked paper's id and title, the typed venue, never the rendered text (a paper's abstract re-renders from its id; values and labels are capped at 300 characters). `record_use(prompt, variables)` files it in the same transaction as the counter and keeps the last 50 per prompt; `use_count` stays the all-time count, so "used 12×" with three rows is by design. The API's render returns the new `use`, the prompt serializer carries `last_use` (a sliced prefetch — one query for the whole list, pinned by a query budget), and `GET /prompts/{id}/uses/` lists the history newest first. MCP `get_prompt` returns `last_use` (the use *before* this call, which is the answer to "what did I run this with last time") and `history=True` folds the full list in — tool count flat at 158. The gallery: a "last used 2 d ago with Theeuwes… · NeurIPS" line per card, a History panel behind the "used n×" chip (fetched only when opened) with **Use again** on every row that had a fill-in — it refills the card (a picked row by id + label, a typed value as typed, a default left as the default). `seed_demo` files three uses for each used demo prompt through the real renderer, only when the prompt has none.

**Why.** PromptLayer's whole pitch is the request log; Raycast and Notion AI forget what a prompt was run with. For a researcher the history is the research trail — "which papers did I summarize for the matrix last week" — and Use again turns yesterday's copy into today's with one click. Ids + labels, not text, keep the table small and honest (the paper's current metadata renders, not a stale copy).

**Alternatives rejected.** Storing the rendered text per use (50 000-char abstracts × 50 rows × every prompt; and a stale copy of the paper); a denormalized `last_use` JSON column on Prompt (a second source of truth for the newest row — the sliced prefetch costs one query for the list); loading every card's history on page load (50 × n rows for a panel most cards never open); a `list_prompt_uses` MCP tool (a flag on `get_prompt` keeps the count flat); "Clear history" (parked in the backlog as 365 — the last-50 cap bounds it today).

**Accepted behaviours.** Use again fills the card and persists nothing until Copy (leaving the page drops it — the remembered values are what Copy last saved); Use again on a row whose picked paper was since deleted leads to the existing "No reference with id …" dialog on Copy, not a silent fallback; a prompt with no fill-ins still files rows — they read "as written" and carry no Use again.

### 2026-09-19 — Prompts slice 3: "Use a prompt with this…" from the paper, the note and the manuscript (#564)

**Decision.** The prompt gallery reads an address: `/prompts?use=<kind>:<id>&label=<title>` shows only the prompts that take that kind of object (`{{paper:reference}}` prompts for a paper, `{{x:note}}` for a note, `{{x:manuscript}}` for a manuscript) with the object already picked into every fill-in of that kind, a banner naming it ("Using *Title* — 2 prompts take a paper, already filled in" with × Show all), no Recent strip (it is a filter, like search), and an honest empty state ("No prompt takes a note yet" → Write one, the form pre-filled with the fill-in). The Reference page's menu, the Library row menu (kebab and right-click), the manuscript page's menu and the note editor's header each offer it in one click. The API gains `GET /prompts/?kind=reference|note|project|manuscript` (whitelisted before it touches a body regex; junk ignored; composes with `q`), documented in the schema; MCP `list_prompts(query, kind)` sends it — Claude asks "which prompts take a paper" and then `get_prompt(id, {paper: 22})`; tool count flat at 158. `seed_demo` carries one prompt per kind so every entry point lands on a hit.

**Why.** #562 made a prompt know what it wants; this makes the object the starting point — a researcher reading a paper thinks "summarize this for the matrix", not "open the gallery, find the prompt, search for the paper". Raycast and Notion AI both start from the prompt; nothing in the field starts from the researcher's object. A deep link keeps one gallery (no second picker UI to maintain), gives the state an address (paste it, go back to it) and mirrors the Inbox `?capture=` capture link.

**Alternatives rejected.** An in-place picker modal on each page listing the matching prompts and copying without leaving (four copies of the picker + the fill-in row; a later slice can add it on top of the address); pre-picking only the first fill-in of the kind (a prompt with two fill-ins of one kind gets the same object in both — accepted and visible in the chips); keeping the arrival separate from the remembered values (Copy saves what it copied, so after arriving with paper 22 the prompt remembers paper 22 for plain visits too — accepted, that is what "last-used" means; and × Show all remounts the cards, so a preset that was never copied is dropped and the remembered values return — accepted); filtering by kind on the client only (Claude needs the same answer over MCP); resolving the label on the gallery from the id (a round trip for a title the source page already has; a missing label falls back to `#id`).

### 2026-09-18 — Prompts slice 2: a use count and a Recent strip — the prompts you reach for sit at the top (#563)

**Decision.** A prompt now remembers how often and when last it was copied: `Prompt.use_count` + `Prompt.last_used_at` (prompts 0002), bumped by `prompts/services.py::record_use` **after** a render succeeds — the API's `POST /prompts/{id}/render/` calls it and carries `use_count` / `last_used_at` back next to `text`, so the SPA patches its list in place (no refetch) and the "used n×" chip and the **Recent** strip (the last five copied, most recent first; hidden while searching; a chip scrolls to its card and flashes it) move at once. A use is not an edit: `save(update_fields=[…])` keeps `auto_now` off `updated_at`, the ETag data version still bumps (a save, not an `update()`), and an `F()` increment survives two copies landing together. A failed render (404 for a missing row, 400 for junk values) counts nothing; the fields are read-only on the API. MCP `get_prompt` now always renders (defaults fill; `values` optional) and so counts as a use — Claude's copies show in the strip like the owner's (tool count flat at 158). `seed_demo` gives two demo prompts a history, only when the prompt was never used, so a re-seed never wipes real usage.

**Why.** Judged against Raycast (a prompt's "last used" sorts the list), Notion AI and PromptLayer (usage analytics per template): a gallery of a dozen prompts is fine alphabetically, a gallery of fifty is not — the three you copy every day belong at the top, and a chip that says "used 12×" is how you find the dead weight. It is also the cheapest honest signal for the run history #565 may add. Counting on the server (not `localStorage`) is what lets Claude's use over MCP and the owner's in the gallery share one number.

**Alternatives rejected.** A `PromptUse` log table with a row per copy (a run history — richer, but a counter + a timestamp answers "which prompts do I reach for" today and is reversible; the log stays a candidate); bumping the count inside `render_with_data` (the service would then write on every read; the view is the one place that knows the render succeeded); a `QuerySet.update(F(...))` (silent to the ETag signals — the gallery would show a stale list); sorting the whole gallery by last use (the alphabet stays the stable order; the strip carries the recency); a Recent strip from `localStorage` (Claude's uses would be invisible).

### 2026-09-16 — Prompts slice 1: typed fill-ins picked from Atlas — `{{paper:reference}}` copies the paper (#562)

**Decision.** A prompt's fill-in can now name what it wants: `{{paper:reference}}`, `{{note:note}}`, `{{project:project}}`, `{{draft:manuscript}}` (a kind after a colon; `{{name}}` and `{{name|default}}` keep meaning a free-text fill-in). In the gallery a typed fill-in draws a **picker** instead of a text box — a debounced search over the existing global search endpoint filtered to that kind (≤ 8 hits, the pick shown as a chip with a ×) — and **Copy renders on the server** (`POST /prompts/{id}/render/` with `{values}`): a reference expands to `Title (Authors, Year).` / venue / `doi:` / `Abstract: …` (lines present only when the field is), a note / project / manuscript to `title`, a blank line, the body / description / abstract; text values and defaults apply as before; each expansion is capped at 50 000 characters. `parse_variables` returns `{name, kind, default}` (an unknown kind is text; the first default and the first non-text kind win when a name repeats); `render_prompt` in models stays pure and `prompts/services.py` is the one place that reads rows — the API action, the SPA's Copy and Claude share it. **MCP: `get_prompt(prompt_id, values=None)` folds the render in** — with `values` it returns the rendered `text` next to the prompt (a typed value is an id from `search`, or any text, which is used as typed); the tool count stays at 158. The demo prompt becomes `Summarize the paper below … {{venue|NeurIPS}} … {{paper:reference}}` so the seed exercises a picker and a default. Last-used values remembered per prompt (#393) migrate: an old string becomes `{text}`, a pick is `{id, label}`.

**Why.** Judged against Raycast, Notion AI prompt libraries and PromptLayer: every one of them has variables; none of them can fill a variable **from the user's own research objects** — that is the one thing a researcher would screenshot here ("my prompt library knows my papers"). Server-side rendering keeps the expansion format in one place for the UI and for Claude, and keeps the SPA from pulling a reference's abstract just to paste it. Folding the render into `get_prompt` rather than adding `render_prompt` honours the owner's "154 tools too much".

**Alternatives rejected.** A `render_prompt` MCP tool (a tool for what is a parameter of fetching a prompt); expanding on the client from the search hit (the hit carries a snippet, not the abstract — and Claude would have had a second format); `{{reference:paper}}` kind-first syntax (name-first reads as a label, matching `{{name|default}}`); pickers for every kind Atlas has (references, notes, projects, manuscripts cover what a prompt about research quotes; files and decisions can join when a use appears — 364).

### 2026-09-16 — Files: the last seam (files without a kind never previewed), and the Files area verdict (#561)

**Decision.** The verdict pass measured the explorer at 420 and 768 px — the empty state, the tree, the selection bar with the filter row, the detail pane with tags — and found no seam (every state fits its viewport; the header's action row wraps with ⌘P alone on a second line, accepted). The pane found the one real gap: a general file created **without a `kind`** — the classic upload form, the demo seed, an older import — was never previewed as text and drew a generic icon, because `is_text`, the content endpoint and the row icon all read `kind` and only the API upload, the write-file path and the folder import set it. Fixed at the root: `Document.save()` derives a blank kind from the file name (before the title — the seed titles its files "Dual-task protocol v3" over `dual-task-protocol-v3.txt`, the classic form strips the extension from the title), a text-only node is `other` whatever its title says, an explicit kind is never second-guessed; `documents/0006_derive_kinds` backfills existing rows with the same rule (5 of the demo's 9 rows were blank — the four text files the seed adds by title, which showed "No in-app preview for this file type" on a fresh install, and the demo figure, which drew a generic icon). Pinned by `test_kind_derivation.py` (save order, the classic bulk upload → text / asset, the migration on rows forced blank).

**Verdict — Files is best-in-field for a single-user research tool.** After #553–#561 a file has a **history** (replace keeps the old bytes as a version with a note; restore is itself undoable; same-name uploads never duplicate; a version **compares** with now as a line diff or, for a table, a cell diff), **tags and a description** managed where they are used (chips in a filter row with counts, rename / colour / merge / delete from the chip, merge-on-clash, an AND filter), **a selection** that zips, moves, tags, duplicates and deletes as one and **drags** as one (Finder's rule for an unchecked row), **time** (sort by name / change / size, relative stamps, a Recent strip, added / changed in the pane), a keyboard for all of it (↑↓←→ ↵ F2 space ⌘A ⌘D Del ⌘P Esc, on the shortcuts sheet), inline previews for images, PDFs, tables and text, a quick-open by name or tag, per-folder zips and templates, the desktop's open-with / reveal, and Claude reading, writing, organizing (`organize_files` by path) and tagging the same tree through the same contract (158 tools). What a Finder / Dropbox / Google Drive / Notion / Obsidian-vault user would still miss, checked against the code: **undo or a trash for a deleted file** — the confirm is the only guard and since #558 the bytes go on commit; the honest design is a soft delete with retention, a slice of its own (backlog 357, first candidate for the return pass); **editing a text file in place** in the pane (the service `replace_content` exists behind write-file and the Studio; the pane only previews — 358); **full-text search inside uploaded files** (global search indexes a document's title and description; a paper's PDF text is indexed, a general file's is not — 359); **Office previews** (docx / xlsx / pptx render nowhere in-app — download or open-with; 360); **cut / paste between folders** (drag and Move to… cover it; ⌘X / ⌘V is a habit, not a capability — 361); **folder-level description and tags** (files carry them, folders do not — 362); **two-way sync with a local folder** (the watched folder imports PDFs into the library only — a desktop feature, 363); comments on general files (the Library's highlights and comments are per paper — not a daily need for a data file). None of these is what a researcher hits every day in a project tree; the first two are the return-pass candidates. The pointer moves to **Prompts** from #562 (the area after Files in the order set at #522); Audit #35 stays due at #568.

**Alternatives rejected.** Setting `kind` at every creation site (four today; the next site would forget again — the model is the one place); deriving from the title first (the seed's and the classic form's titles lack the extension the file carries); leaving the seed rows as they were and fixing only the demo (a fresh install of an older release has the same rows — the migration reaches them).

### 2026-09-16 — Files slice 7: the selection behaves like a Finder selection — drag it, duplicate it, and Claude organizes files by path (#560, backlog 355)

**Decision.** Three verbs that a file explorer is judged by. **Drag the selection:** dragging a checked row carries every checked file (the drop banner says "Move 3 files into this folder"); an unchecked row dragged while others are checked moves alone — Finder's rule; a multi-move is one bulk request, a single move stays the per-file PATCH; ids already in the target are dropped from the move. **Duplicate:** from the file menu ("Duplicate", ⌘D), the selection bar ("Duplicate") or the key — the copy lands next to its original under the same numbered name a same-name upload would get (`rt-2.csv`, `rt-3.csv`, per folder), carries the description, tags, kind and content type, and starts a history of its own (no versions); the copies become the selection so a second verb (move, tag) acts on them. `documents/bulk.py::duplicate_document` reads the bytes and saves them under a **new storage name** — since #558 a deleted row unlinks its own file, so two rows must never share one (pinned: delete the original, the copy's bytes stay). `untag` joins the bulk verbs (API / MCP only: the pane already removes a tag per file). **Claude:** `organize_files(project, paths, action, folder, tag)` — move / tag / untag / duplicate / delete by tree path (`rel_path` from `list_project_files`), a folder by its path ("Data/Pilot", walked from the tree's flat folder rows, "" = the root), a tag by name any-case; unknown paths and folders name themselves in the error; manuscript sources come back in `skipped`. **The fold (158 tools, flat):** `export_bibtex` is gone — its own docstring already pointed at `export_references(fmt="bib")`, which takes the same ids / project and every workbench filter besides; the skill and README say so. This is a different fold from the two named at #559 (both would have moved a *read* into another tool's shape; this one removes a strict subset). A **Files** group joins the shortcuts sheet (↑↓ ←→, ↵, F2, space, ⌘A, ⌘D, Del, ⌘P, drag, Esc) — the explorer had ten bindings and no card.

**Why.** A selection that cannot be dragged is a checkbox list, not a selection; Finder, Explorer and every IDE tree move what is checked. Duplicate is the "start from this one" gesture researchers use for protocols and analysis scripts, and it had no home. Backlog 355 waited only for a slot.

**Alternatives rejected.** A `POST /documents/{id}/duplicate/` action (the bulk service already carries the selection semantics; one code path for one file and many); "Copy of x" naming (the numbering scheme already exists for uploads, and sorts next to the original); copying the version history onto the copy (a copy starts its own story — Finder and Git agree); an MCP `organize_files` that takes ids (Claude reads paths, not ids, from a tree listing; ids would push a lookup onto every call); folding `read_project_file` or `list_projects` instead (see above).

### 2026-09-16 — Files slice 6: tags managed where they are used, and an AND filter (#559, backlog 354)

**Decision.** A file tag is managed from its chip in the explorer's filter row, never from the admin: right-click a chip (or, once it is active, its ⋯ — so touch and keyboard users reach the same menu) for **Only this tag** / **Show every file**, **Rename…**, **Colour…** (the eight-hue palette, the current hue marked), **Merge into…** (the project's other tags, each with its file count) and **Delete tag…** (danger, the count as the hint; the confirm says "It is on n files; they keep their other tags"). A rename into a name another tag already has — any case, because that is the tag the explorer's "New tag…" would have reused — does not fail: it offers the merge ("A tag named “key-paper” already exists — Merge “methods” into it? 2 files will carry “key-paper” instead"). Several chips at once mean *files carrying every one of them* (AND): the count reads "1 of 9 · all 2 tags" and a Clear sits beside it. After a rename / merge / delete the active filter follows the name (substituted, folded into the target, or dropped), so the tree never shows "0 of n" with no chip left to clear; every mutation refreshes the tree and the tag pool.

Backend: `documents/tags.py` — `rename_tag` / `recolour_tag` (`clean_name`: whitespace-collapsed, ≤ 60, iexact clash → `TagError` naming the existing tag; `clean_color`: `#rrggbb` or empty), `merge_tags` (through the M2M manager — `target.documents.add(*source.documents)` then `source.delete()` — so `m2m_changed` bumps `updated_at` and the data version; a file carrying both simply loses one chip; self / cross-project refused), `delete_tag` (returns the count), and `tag_names` + `filter_by_tags` (repeated `?tag=`, ≤ 10 names, one join per name = AND, `distinct`) shared by `GET /documents/` and `GET /projects/{slug}/tree/` (folders always listed). API: `TagSerializer` gains read-only `count` (annotated on the viewset) and a `validate()` that owns the (project, name) rule (the generated unique validator is switched off so the 400 names the clashing tag) and refuses re-parenting a tag into another project (the #554 invariant — a document may only carry its own project's tags); `POST /tags/{id}/merge/ {into}` (self → 400, another project's tag → 404). **MCP (158, flat):** `list_documents` — one of the Phase 6 initial tools in CLAUDE.md §5 — is folded into `list_project_files(project, tag="")` (the tree already carried tags, description, sizes and dates since #554 / #557; `tag` comma-separates several and a file must carry them all — a name containing a comma is the one thing the MCP boundary cannot express, documented), and the freed slot is `manage_file_tag(project, tag, rename | color | merge_into | delete)` — exactly one verb per call, tags matched by name any-case, `delete` reports the file count from the tag list first because `DELETE` answers 204 without a body.

**Why.** Tags only earn their place if they stay tidy, and tidying meant the admin or the classic Tags page until now — a rename of "ms" to "manuscript" that collides with an existing tag failed with a generic uniqueness error (or, in another case, the database constraint) and no way through. Finder, Zotero and Obsidian all manage tags from the tag itself; the AND filter is what Finder's tag sidebar cannot do and what a researcher asks for first ("the figures that are also key results"). The merge-on-clash flow turns the commonest failure into the commonest wish.

**Alternatives rejected.** A tag management page (a place to go instead of the place the tags are); a submenu component for Colour… / Merge into… (the menu opens a second menu at the same point — no new component); nested `?tag=a,b` in the API (a comma is legal in a name; repeated parameters are the HTTP way); keeping `list_documents` and adding a 159th tool (the owner's "154 too much" stands — the fold path was already logged under 355, which now needs its own fold: `get_project_overview`'s counts vs `list_projects`, or `read_project_file` absorbing the raw-version read, are the candidates).

### 2026-09-16 — Audit #34: the test suite's media, deletes that clean up, and the phone seams (#558)

**Decision.** The every-ten-cycles look at #549–#557: dependencies clean, `scripts/audit.sh` green, every new Files surface (versions, replace, diff, tags, bulk, archive, tree) and the Today logbook probed for auth, bounds and junk — no finding in the window's own slices (the report is in AUDITS.md). One finding from the inside: the tests wrote every saved file into the working tree's `media/` (32 471 files, 571 `project-N` directories) — `conftest.py` now gives every test a temp `MEDIA_ROOT`. Five queued items closed: **356** a `post_delete` hook on `Document` and `DocumentVersion` unlinks the storage file on commit, and `manage.py prune_media` (dry run by default, `--apply`) removes files under the documents app's two prefixes that no row references — it cleared the 32 448 orphans; **343** the first OA address goes through `check_url` like every hop (one gate at the top of the loop; the OA fixture stubs the resolver by default); **349 / 350 / 351** the Projects header wraps, the overview grids' cards may shrink (`[&>*]:min-w-0`), the constellation draws labels only from a 640-px canvas up. **344 and 345 are not audit work** (a Today Trash, Logbook retention) — they stay queued for the Today return pass.

**Why.** An audit is where the code that "always worked" gets asked what it leaves behind: here, bytes — on every delete since the first upload, and on every test run since the first file test. Both fixes sit at the root (a signal, a fixture) with a sweep for what accumulated; the phone seams were measured, not guessed, and each was one class.

**Alternatives rejected.** Deleting storage files inside `perform_destroy` / `bulk_documents` only (a folder or project cascade would still leak — the signal sees every path); unlinking immediately rather than on commit (a rolled-back delete would lose its bytes); a prune that considers every file under `MEDIA_ROOT` (reference PDFs and manuscript assets belong to other apps and other rows — the two documents prefixes are the honest scope); a session-scoped temp media root (per-test is simpler and pytest-django's `settings` fixture already fires `setting_changed`).

**Follow-up found by the full suite.** `config/urls.py` bound `document_root=settings.MEDIA_ROOT` at import, so `override_settings(MEDIA_ROOT)` never reached the `/media/` view and `test_media_served_with_debug_off` had only been passing because an orphan `manuscript-1.pdf` sat in the real `media/`. The view now reads `MEDIA_ROOT` per request — the same fix serves the desktop, whose data folder is decided at launch.

### 2026-09-16 — Files: time in the explorer — sort, stamps, a Recent strip (#557)

**Decision.** Tree rows carry `created_at`, `updated_at` and **`modified_at`** — when the bytes last changed, which is the newest version's stamp (a version is filed at that moment) or the creation, *not* `updated_at`, which a tag, a description edit or a move also bumps; the annotation rides on the existing versions join (`Count(distinct)` + `Max`). The explorer gets a **sort** on the Explorer header (Name / Last change / Size; folders always first by name; newest change first, largest first, ties by name; remembered in `localStorage` under `atlas-files-sort`); under Last change every row shows a relative stamp in place of its size, and every row's title carries the absolute "changed … · added …"; the pane gets an "added 3 d ago · changed just now" line with the absolute times on hover; a collapsible **Recent** strip above the tree lists the six last-changed files across the project (only when the tree has six or more files, hidden under a tag filter, its open state remembered) and clicking one opens every folder above it and selects it. MCP unchanged (158): `list_project_files` returns the tree, so its rows carry the stamps for free; the docstring names them. **Found first:** an inline-text node (a `write-file` from Claude, an editor-created file) had `file_size` 0 — the Size sort put a 900-byte note last and rows showed no size; `Document.save()` now sizes inline content in bytes. A manuscript source has no versions and only the Studio's mirror writes it, so its `updated_at` is its change time. Post-ship: a control inside the tree (the sort select, a tag chip, the bar, a checkbox, a Recent row) now keeps its own keys — the tree's arrow / Enter / space / typeahead handling ran on them too, so ArrowDown on the focused select moved the tree focus and Enter on a chip was swallowed (older than this slice for the chips and the bar, acute with the select); one `Intl.DateTimeFormat` instead of two per row per render; a data migration (documents 0005) backfills the size of inline-text nodes that already existed; ⌘P opens the folders above the picked file like the Recent strip. Also checked and not needed: the explorer fits at 420, 640 and 768 px with no overflow (tree stacked above the pane), so the narrow-width pass the pointer listed is retired without a slice.

**Why.** Every file manager sorts by date and size and shows "modified"; the explorer sorted by name only and showed no date anywhere, so "which export did I make on Tuesday" meant opening History on each. `updated_at` would have been the easy answer and the wrong one — tagging five files on Monday would make them "changed Monday" — and the version stamps already carry the true answer. Relative stamps on rows only under the Last-change sort keep the default view quiet (rows are already name · dots · version · size · menu).

**Alternatives rejected.** A permanent date column (noise for the common case; the title and the pane carry it); sorting folders too (folders by name is what Finder and VS Code do and what keyboard users expect); a server-side "recent" endpoint (the tree already has every stamp; six rows sorted client-side cost nothing); a shared relative-time helper across pages (`Inbox.tsx` and `ProjectOverview.tsx` each have a local one with different wording — a refactor for another day, not this slice).
...
375. Files: BibTeX highlighting for a `.bib` in the pane editor (codemirror-lang-latex highlights .tex only; a small stream parser for @entries and fields) and JSON / Python highlighting for data scripts (idea added by #577).
374. Files: a trash for folders — today a deleted folder moves its files to the project root and its subfolders go with it; a folder trash would restore the whole subtree at its path (idea added by #576).
373. Diagnostics: "Download the report" next to Copy report — the same text as a `.txt` (a bug tracker wants a file; a chat wants a paste) (idea added by #575).
372. Diagnostics: a finding's age — a small store of first-seen stamps per finding id so the verdict block can say "since Tue" and the brief can tell a fresh problem from one that has sat for a month (idea added by #575).
371. Diagnostics: "Run the doctor" on the page for server installs — spawn `manage.py doctor` and stream its lines into a panel, so the shell-side checks a request cannot see (Redis reachable, worker alive and on current code, migrations applied) reach the stranger who cannot open a shell; pairs with 252 (make the doctor print the findings and keep only the shell-side checks inline) (idea added by #575).
370. ~~Diagnostics: the Access section's twelve most recent events are almost always the owner's own logins, so the rows that matter (the rejected keys from Sep 15) fall out of `recent(12)` entirely — list problems first (failed logins, lockouts, rejected keys), then the last few logins, or give the section a "problems only" toggle; the #572 verdict row names the last problem, the list under it should show it (idea added by #572).~~ — shipped by #573
369. ~~Diagnostics: the three "Update feed" rows share one label and nothing says which address the app tries first or that the others are fallbacks — label them "Update feed · tried first / fallback / fallback", and collapse the unprobed ones into one row until the probe runs (idea added by #572).~~ — shipped by #574
368. Prompts: import / export of the gallery as a file (JSON or a markdown bundle) for sharing with a colleague or moving between installs — prompts are rows in the backup and over the API today (idea added by the Prompts verdict, #569)
367. Prompts: a ⌘K "Copy a prompt…" verb (pick a prompt, fill in, copy without leaving the page) and a desktop global hotkey for it — Raycast's one habit the gallery page cannot match (idea added by the Prompts verdict, #569)
366. Prompts: versions of a prompt's body with a diff and a restore (an edit overwrites today; the notes' `NoteRevision` shape — snapshot the pre-save body, coalesce within 10 minutes, keep 50 — fits), the first candidate for the Prompts return pass (idea added by the Prompts verdict, #569)
365. Prompts: "Clear history" / delete one use from a prompt's History panel, and a `DELETE /prompts/{id}/uses/{use_id}/` (the last-50 cap bounds the table today) (idea added by #565)
364. Prompts: typed fill-ins for more kinds — `{{file:document}}` (a text file's content), `{{decision:decision}}`, `{{question:question}}` — and `[[Note]]` / `@key` mentions inside a prompt body expanding the same way as a typed fill-in at copy time (idea added by #562)
363. Files: two-way sync of a project's tree with a local folder on the desktop (the watched folder of #406 imports PDFs into the library only) — the explorer's rows would gain "on disk" state and conflicts a Studio-like mirror already handles for manuscripts (idea added by #561)
362. Files: a description and tags on folders (files carry both since #554; a folder is a bare name) — the filter row would count folders too (idea added by #561)
361. Files: cut / copy / paste between folders (⌘X ⌘C ⌘V on the tree; drag and Move to… cover the capability today) (idea added by #561)
360. Files: Office previews — docx / xlsx / pptx rendered in the pane (a server-side conversion or a client library; today they download or open with the system app) (idea added by #561)
359. Files: full-text search inside uploaded files — global search indexes a document's title and description; index the text of text / CSV / PDF general files the way a paper's PDF text is (`text__body`) (idea added by #561)
~~358. Files: edit a text file in place in the pane (`replace_content` exists behind write-file and the Studio; the pane only previews) — a CodeMirror editor with save-as-version and the same diff on save (idea added by #561)~~ — shipped at #577 (a CodeMirror editor in the pane, a note per save, the diff strip after a save, `filed` on the save and write-file answers)
~~357. Files: undo / a trash for a deleted file — the confirm is the only guard and since #558 the bytes are unlinked on commit; a soft delete with a retention window (rows + files kept, "Deleted" section, restore, `prune_media` sweeps expired ones), the first candidate for the Files return pass (idea added by #561)~~ — shipped at #576 (a Trash under the explorer, thirty days, restore / forever / empty, `organize_files` restore / purge)

~~356. Documents: delete leaves the storage files behind — no `post_delete` cleanup for `Document.file` or `DocumentVersion.file`, so every delete (single, bulk, folder) orphans bytes under `MEDIA_ROOT`; a signal that removes them (and a sweep for the existing orphans) belongs with Audit #34 since it touches every delete path (idea added by #556's review)~~ — closed by Audit #34 (#558)
~~355. MCP: organize files in bulk — move / tag / delete by ids or paths (the API's `documents/bulk/` exists since #556); would be the 159th tool, so it waits for a fold — the `list_documents` → `list_project_files` fold was spent on `manage_file_tag` by #559; next candidates: `read_project_file` absorbing a raw-version read, or `list_projects` carrying the overview counts (idea added by #556)~~ — shipped by #560 as `organize_files` (by path) after folding `export_bibtex` into `export_references`
~~354. Files, tag management from the explorer: rename / recolour / delete a tag (with the count of files it would leave) from the filter row's chip menu, and a multi-tag filter (AND) — today a tag is made and applied from the pane, and managed in the admin (idea added by #554)~~ — shipped by #559 (rename / recolour / merge / delete from the chip menu + ⋯, the AND filter, `manage_file_tag`)
...
~~351. The dashboard hero at phone width: the active project's caption (the constellation header's label) sits over the "n things need you" headline at 420 px — give the caption its own line below `sm` (idea added by #552's route sweep)~~ — closed by Audit #34 (#558)
~~350. The project overview at phone width: the page measures 608 px wide at 420 (the pulse strip, the glance row or the constellation header does not collapse) — find the fixed-width child with the #551 overflow probe and make it wrap or scroll (idea added by #552's route sweep)~~ — closed by Audit #34 (#558)
~~349. The Projects index at phone width: 436–441 px of scroll width at 420 (a card row or the header's button group) — the same probe, the same fix (idea added by #552's route sweep)~~ — closed by Audit #34 (#558)
...
252. The doctor's worker/redis, static-css, optional-components (Tectonic/Piper), and media+api-key blocks are still inline in handle() like the desktop block was before #208. If handle() keeps growing, the same extract-to-_check_X() treatment (a small ordered list of self.* section methods) would keep it a readable table of contents. Low priority — only worth it if another section accretes sub-checks (idea added during #208) Since #572, `core/diagnostics.py::findings` is the second home of several of these checks (engine, API key, media writable); the refactor should make the doctor print the findings and keep only the shell-side checks (Redis, worker, migrations, CSS) inline.
