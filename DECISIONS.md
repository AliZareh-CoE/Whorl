...
171. Density pass on the documents/folder tables (classic) — the library index is done (#165); the per-project documents listing + folder tables are the remaining #25 table surfaces; same widen + tighter row rhythm (idea added during #165 library pass)
170. Sortable columns on the library index — now that it's wide and dense, clickable Year/Cited headers (querystring ordering, no JS) would make the bigger table scannable (idea added during #165)
...
173. Lucide icons on the project context subnav (Overview/Plan/Documents/Literature/…) — the primary sidebar now has the icon language; the per-project subnav is the next surface to match (idea added during #161)
