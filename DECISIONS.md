...
## Backlog
250. The API's DocumentViewSet.raw endpoint (api/views.py) picks its inline content_type purely from the file *extension* (e.g. `.png` → image/png) and never sniffs the bytes — the same content_type/bytes-mismatch class #249 just closed for the HTML preview path. Reusing documents.models.sniff_image_type there (serve inline only when the magic bytes confirm the extension-implied raster type, else 404/text) would make the two inline paths consistent. Low priority — nosniff already blocks execution and these are single-user uploads — but it's the obvious next consistency pass (idea added during #249)
