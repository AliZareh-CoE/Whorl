...
225. If #224 doesn't fully fix the owner's Windows launch, the next suspects (read atlas-server.log to confirm): Postgres refusing to run elevated, a slow first-run initdb+migrate exceeding the 180s wait (main.rs loads the URL even when wait_for_port times out — it ignores the bool), or a missing VC++ runtime for the zonky binaries. main.rs should also load a friendly bundled error page (pointing at the log) instead of the raw browser refused-to-connect when wait_for_port returns false (idea added during #224)
...
223. AUDIT #21 measured documents_index at 13 queries warm (root == nested — the breadcrumb is query-free) and project_literature at 6, but neither has a django_assert_max_num_queries budget guard, so a future per-row N+1 (added to _doc_row, the island props, or a filter) could slip in unseen. A cheap budget test on both views would lock the counts in, like the weekly_review(8) guard already does (idea added during AUDIT #21)
...
217. The review page's project-name chips (papers/milestones/etc. on the cross-project view) are now slug-bearing but still render as plain text — they could become Links to the project overview for one-click drill-down. NOTE (from #216 follow-up): the paper/note rows are already `<Link>`s, so their chips can't nest a second Link; only the milestone/decision/experiment rows (plain divs) could link cleanly — partial = inconsistent, so probably skip unless the rows are restructured (idea added during #216)
