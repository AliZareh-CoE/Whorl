...
357. Files: undo / a trash for a deleted file — the confirm is the only guard and since #558 the bytes are unlinked on commit; a soft delete with a retention window (rows + files kept, "Deleted" section, restore, `prune_media` sweeps expired ones), the first candidate for the Files return pass (idea added by #561)
