...
212. Persist the documents tag filter per-project (the literature filters #187 used a single global session key, fine because their values are global enums; a tag pk is per-project, so remembering it globally would surface a stale/foreign tag on another project). Needs remembered_choice to take the optional namespace suffix from #194 (e.g. session_key=f"doc_tag:{project.slug}") + validating the pk belongs to the project. Folder stays navigational, not persisted. (idea added during #187)
