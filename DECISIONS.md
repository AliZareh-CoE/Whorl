...
149. Mood-flavored voice on top of stage — pet_state() also has the weekly mood (sleeping/content/happy/thriving); a small volume or noise tweak per mood would layer onto the stage delivery (the syn_config plumbing now exists) (idea added by cycle 138)
...
151. Central escape discipline for the glue — audit #14 found unescaped innerHTML in the research panel; a tiny shared render helper (or moving the panel to the React island world where escaping is automatic) would make the whole glue immune by construction instead of by review (idea added by cycle 140, from AUDIT #14)
