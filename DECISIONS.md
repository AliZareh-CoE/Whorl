...
197. Pass the cached folder list into selectors.folder_tree too — it still issues its own project.folders.all() (1 query) separate from the documents_index cache; an optional pre-fetched arg would fold the last duplicate, but folder_tree has several callers so it needs care (idea added during #181)
...
196. Tighten the base_wide guard to also fail if a listed wide page reintroduces a hand-rolled max-w-* main_class block (double-setting the width); right now it only checks the extends line (idea added during #195)
