...
235. AUDIT #22 idea: the document_preview inline endpoint relies on a content_type stored at upload time (browser-reported) — fine for single-user, but a belt-and-suspenders hardening would sniff the actual file magic bytes for the image whitelist (e.g. verify a real PNG/JPEG header) before serving inline, so a forged content_type can't even reach the inline path. Low priority — nosniff already neutralizes execution; revisit only if previews ever face untrusted uploaders (idea added during AUDIT #22)
