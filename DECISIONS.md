...
147. Audit-sweep output as a CI artifact too — `make audit` prints its probe results to the log only; tee it to a file and fold it into the same failure() upload so a red audit job ships its evidence (completes the #112 idea) (idea added by cycle 136)
