...
203. A tiny `doctor` warning if the updater pubkey is still the placeholder (surfaced by AUDIT #19 alongside D3) — so a release isn't tagged expecting auto-update to work when signing was never set up (idea added by AUDIT #19)
