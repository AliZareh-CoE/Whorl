...
- **Slice in progress:** — (NEXT: **Owner idea #30 — FILE WORKSPACE / IDE EPIC** outranks the backlog. Plan of record: docs/plans/2026-06-12-file-workspace-ide-epic.md (9 slices, 4 owner decisions: unify tree / full Tauri / terminal / OSS-first). Slice 1 = unified model+migration+contract bridge — REDO from clean base: do it as small green-pushable sub-steps (1a paths+ProjectFile fields+schema migration, 1b Manuscript.root_folder/main_file, 1c data migration+contract bridge+tests) given container instability. Backlog alongside: #148 icons / #149 mood voice / #150 reset layout / #155 card-title / #158 triage undo; AUDIT #15 at 150)
- **⚠ RECOVERY 2026-06-12 (2nd container rollback this session):** restart rolled local .git + git-proxy + DB volume back to fb85a2e (cycle 104); a worker's stale slice-1 attempt layered on top. Real remote (GitHub) was intact at 192 commits — recovered via: git fetch → stash stale worker changes → ff-merge to origin (4d69a1f) → revive dockerd/postgres/redis/runserver/huey → migrate (only 0008 pending) → REVERTED the broken slice-1 WIP 4d69a1f (model fields w/o migration = broken main) back to green cycle-147+plan state (5eaf4d7, 564 tests green). WIP's paths.py + field design recoverable from history at 4d69a1f for the redo.
- **Last completed slice (cycle 147, Backlog #157, UI/UX):** INLINE TRIAGE — attention rows triage in place on BOTH shells: SPA project-select + file/dismiss (react-query PATCH, query invalidated), classic compact form → notes:triage which now honors a safe `next` (offsite rejected, 2 new tests). 9-check live battery both shells ALL PASS. 564 tests.
