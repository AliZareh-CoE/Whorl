...
- **Slice in progress:** — (next: Owner idea #4 — auto-download open-access PDFs on reference add)
...
- **Next 3 slices:**
  1. Owner idea #4 — auto-download open-access PDFs (Unpaywall/arXiv) on reference add
  2. Owner idea #5 — NLP helpers (keyword extraction → tag suggestions first)
  3. UI/UX cycle (rule: ≥2 per 10 cycles) — consistency + interaction polish pass
- **Broken:** nothing
