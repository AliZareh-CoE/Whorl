...
- **Slice in progress:** — (next: CM6 Slice C part 2 — port the glue + cut the template over [cycle 125])
- **Last completed slice (cycle 124, CM6 migration Slice C part 1):** cite B1/B2 ported into the island — whole-library cite completion (fetch on mount, "+" unlinked, auto-link on accept) + a citeCheckDiagnostics linter merging unknown-cite warnings with compile diagnostics. Harness-verified. CM5 STILL LIVE; the glue cutover (port the 1086-line file's ~30 CM5 calls to the adapter, flip the template, remove 11 cdnjs tags, full in-app verify) is split to cycle 125 — not rushing a big-bang on the key file (a CM5-compat shim proved the wrong abstraction).
