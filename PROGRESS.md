...
- **Slice in progress:** — (next: B3 research side panel [UI/UX] OR slice 7 SyncTeX OR slice 10 templates [cycle 113])
- **Last completed slice (cycle 112, beyond-Overleaf B2):** live cite-check — unknown \cite{} keys get an amber lint squiggle (merged into the diagnostics lint channel; complete tokens only, calm); a "Unknown citations" bar offers add-by-DOI inline (reuses by-doi → refresh library → re-check). Fixed a construction-time TDZ via an editorReady gate. Browser-verified flag/clear.
