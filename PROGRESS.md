...
- **Slice in progress:** — (next: Overleaf-UI slice 2 — Split.js resizable panels + top toolbar [cycle 127])
- **Last completed slice (cycle 126, CM6 CUTOVER — Backlog #135 COMPLETE, #114 CLOSED):** the glue (static/js/latex-editor.js) is now an ES module on the CM6 island adapter; hand-rolled snippet walker + 4 hint functions + cite-check scanner DELETED (island + lang-latex native, hover tooltips free); all 15 cdnjs CodeMirror tags removed — ZERO CDN editor assets (offline-editor bug closed); keymaps Default+Vim. Verified live with a 26-check battery: ALL PASS.
