...
- **Slice in progress:** — (next: CM6 GLUE CUTOVER — port latex-editor.js to the adapter + flip the template [cycle 126])
- **Last completed slice (cycle 125, CM6 Slice C part 2a):** adapter cutover-complete — onCiteCheck added (unknown-cite keys → the glue's missing-citations bar), harness-verified. SURVIVED a container snapshot rollback (workdir+DB reverted to ~cycle-105 state): recovered via git fetch + reset --hard origin (all cycles were pushed — push discipline pays), npm reinstall, service revive, redid the one uncommitted edit. NOTE: DB lost dogfood records from later cycles (demo data, acceptable).
