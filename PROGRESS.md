...
- **Slice in progress:** — (next: Overleaf-UI top toolbar consolidation [cycle 128])
- **Last completed slice (cycle 127, Overleaf-UI Split.js panels):** real drag handles between file sidebar | editor | PDF pane (Split.js MIT via the island bundle, per Owner #28); per-layout size persistence, split rebuilds on preview toggle, drag-end re-measures CM6 + re-renders the PDF. Live-verified: drag 558→742px, persisted across reload, editor functional.
